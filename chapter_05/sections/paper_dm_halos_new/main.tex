...
\cite{2012ApJ...753...83A, 2016ApJ...825...69M, 2016ApJ...820....8S, Lefaucheur:2017ajt, 2019JCAP...11..045C, 2020MNRAS.492.5377L, 2020MNRAS.495.1093H, 2021RAA....21...15Z, 2021MNRAS.505.5853G, 2022A&A...660A..87B, 2023JCAP...07..033B, 2023MNRAS.521.6195M, 2023RASTI...2..735M, 2023MNRAS.520.1348G}}
...
perform several consistency tests in appendices.
...
\begin{itemize}
    \item Galactic {(dominated by millisecond pulsars at high latitudes~\cite{smith2023third, 2020ApJS..247...33A, Fornasa:2015qua, Hooper:2024avz, Cholis:2014noa, Caraveo:2013lra})}: pulsars, binaries, star-forming regions, supernova remnants, pulsar wind nebulae, globular clusters, and novae;
    \item Extragalactic ({mostly AGNs~\cite{fossati1998unifying, 2020ApJS..247...33A, Fornasa:2015qua, Dermer:2009zz, Fermi-LAT:2014ryh, Ajello:2015mfa, Lisanti:2016jub}}): blazars, quasars, starburst galaxies, normal galaxies, radio galaxies and radio sources, Seyfert galaxies, and non-blazar active galaxies.
\end{itemize}
%
%% aure: working here
%
{This choice is made to group sources based on their fundamentally different physical origins and radiation environments. By separating the magnetospheric emission characteristic of Galactic pulsars from the jet-driven processes of extragalactic AGNs, we ensure that the classification reflects the intrinsic physical diversity of the $\gamma$-ray sky, which manifests in distinct spectral signatures.}
...
% \aure{for mDM=30GeV the number of subhalos in the full range is 64, not 27...}
% OLD paragraph 1:
% The expected number of DM subhalos of \new{36.2} \old{15.4} for this DM mass \new{in the full range of parameters} \old{in our ROI} is comparable or slightly larger, e.g., to 16 DM subhalo candidates reported in ref.~\cite{2019JCAP...07..020C},
% \new{if we take into account improved sensitivity between the 4FGL-DR4 catalog (14 years of observations) vs the 3FGL catalog (8 years of observations).}
% \new{
% Indeed, if we assume that the DM subhalo-like sources are astrophysical sources, which scale as
% $N(> S) \propto S^{-1.5}$ as a function of their flux (in the Euclidean regime of constant volume density) and that the detection threshold scales as  $S \propto t^{-0.5}$ as a function of time, then for 36.2 subhalo-like sources in 14 years of observations corresponding to 4FGL-DR4 (in the full range of parameters), one can expect
% about 27.4 sources in 8 years of observations corresponding to the 3FGL catalog used in ref.~\cite{2019JCAP...07..020C}.
% Provided that 16 is within about 2.2 sigma statistical fluctuations from 27.4, we find that the two methods give consistent estimates of the number of possible DM subhalos.}
% \new{Alternatively, we can assume that the sources are in fact DM annihilating subhalos.
% In our baseline model $dN/dJ \propto J^{-3}$ (black dashed line in figure~\ref{fig:Jfactors}), i.e., the total number of observable subhalos above a flux sensitivity threshold is $N_{\rm sub} \propto J^{-2} \propto t$, where we have used the fact that for a fixed $\sigmav$, the flux $S \propto J$.
% In this case, 36.2 detectable subhalos in 14 years of observations would correspond to $36.2 \times 8 / 14 \approx 20.7$ subhalos in 8 years of observations, which is also consistent with the 16 subhalos assumed in ref.~\cite{2019JCAP...07..020C}.
% }
% END OLD paragraph 1
The expected number of DM subhalos of \new{36.2} \old{15.4} for this DM mass \new{in the full range of parameters} \old{in our ROI} is comparable to the 16 DM subhalo candidates reported in ref.~\cite{2019JCAP...07..020C},
\new{once the improved sensitivity of the 4FGL-DR4 catalog (14 years of observations) relative to the 3FGL catalog (8 years of observations) is taken into account.}
\new{
...
\new{Alternatively, if the sources are DM annihilating subhalos, our baseline model has $dN/dJ \propto J^{-3}$ (black dashed line in figure~\ref{fig:Jfactors}), which gives $N_{\rm sub} \propto J^{-2} \propto t$, where we have used $S \propto J$ for fixed $\sigmav$.
...
\old{
This comparison should be taken with a grain of salt, as the calculation in ref.~[29] was performed for the 3FGL catalog (8 years of data) compared to the 4FGL-DR4 catalog (14 years of data) in our work with the J-factors based on the mass estimate within the tidal radius (cf. $J_{\rm tidal}^{\Msub}$ in Section~3.1) compared to the J-factor integrated within $0.1^\circ$ from the center based on the maximal velocity used in table~2.}
...
\midrule
10 & 0.0 & 0.00 & 0.00 & 0.0 & 0.0 & \old{2.83} \new{1.91}& \old{6.56} \new{4.42}& $5.6 \times 10^{-26}$\\
...
1000 & 0.0 & 0.00 & 0.00 & 0.0 & 0.0 & \old{3.37} \new{2.06}& \old{13.75} \new{8.61}& $4.7 \times 10^{-24}$ \\
\bottomrule
\end{tabular}%
}
\caption{
...
\caption{
\new{Same as Table \ref{tab:Nsub_values}, but for the $\rm J_{tidal}^{Msub}$ case.} 
%\aure{Check caption. the bound values should be correct now}
}
\label{tab:Nsub_values_Msub}
\end{table}
...
\begin{figure}
    ...
    Gray band: 95\% containment of the upper limit due to statistical fluctuations of the distribution of sources, \new{the effect of masking the 4FGL-DR4 sources is added to the upper values of the band (appendix~\ref{app:sim-ps})}.
    ...
    Dashed orange line: limits derived by Coronado-Bl{\'a}zquez et al. (figure \old{5} \new{12} upper panel%, black line
    )~\cite{2019JCAP...11..045C}. 
    Solid green line: the limits derived by Butter et al. (figure 11,  \new{the 0.8 threshold limit for $|b| > 10^\circ$}% solid green line
    ...
    %\dima{For consistency argument, I'd suggest to use the high-latitude limit of Butter et al and to add the 3FGL b-bbar limit from Coronado-Blázquez et al, https://arxiv.org/pdf/1906.11896 (blue line on the upper panel of Figure 12). The last limit is a factor 1.5 or so more constraining than our corresponding limit in red dashed line in our Figure 12, which is consistent with the fact that we have 64 subhalos (instead of 16 in 1906.11896) plus a small improvement for using 14 years of data instead of 8. To avoid making the plot too busy, we could remove the zero subhalo limits (or keep only ons of them, since they are consistent with each other).}
    %\aure{I have updated the figure, and the caption.}
    }
    \label{fig:bounds-bb}
\end{figure}
...
\begin{figure}
    ...
    \caption{
    %\new{Same as Figure \ref{fig:bounds-bb}, with added }
    ...
    %\dima{I would also suggest to add the 3FGL b-bbar limit from Coronado-Blázquez et al, https://arxiv.org/pdf/1906.11896 (blue line on the upper panel of Figure 12) to directly compare with the red dashed line in this plot (would be useful as a consistency check of our limits, also in reply to p.2 of the referee report). Let's also add high-latitude conservative (0.8 threshold) limit of Butter et al.}
    }
...
%%%%
\old{For comparison we show the limit obtained by assuming that there are 16 DM subhalos in the 3FGL catalog by the purple solid line~[27].
We notice that our limit for $\mdm = 30$~GeV 
is close to
the limit derived in ref.~[27]. Although the upper limit on the number of DM subhalos in our analysis (27.3) is larger than the number of DM subhalo candidates (16) assumed in~[27] following the analysis of ref.~[29].
Our limit is slightly better due to longer observation time (14 years vs 8 years), which results in better flux sensitivity.
For other DM masses, 
our limit is better than the limit reported in~[27] due to smaller upper limits on the number of DM subhalo candidates.
All of these upper limits on DM subhalo candidates, on the other hand, are larger than 1. As a result, the constraints on the DM annihilation cross sections are worse than the best possible constraints corresponding to zero observed DM subhalos, shown by the purple dash-dotted line~[27] and the orange dashed line~[28] in figure~11.
Our limits are also significantly better for most of DM masses than in ref.~[31] due to the very conservative approach in estimating the number of DM subhalos adopted in ref.~[31], where the number of possible DM subhalo candidates, e.g., for $\mdm = 30$~GeV is estimated to be 281 in the most conservative scenario.
}
...
\new{The limits for J-factor integration
...
% OLD paragraph 2:
% \new{The solid gray line shows the limits derived in ref.~\cite{2023JCAP...07..033B} for the J-factor integrated up to $0.5^\circ$ radius.
...
%\aure{check again these numbers, should we use the new numbers?}
...
\begin{figure}
    ...
    \caption{Detection efficiency for the $b\overline{b}$ annihilation channel simulations. {Solid lines denote the baseline efficiency used in the analysis, derived from simulations that disregard known 4FGL sources. Dashed lines illustrate a conservatively corrected efficiency, representing a worst-case scenario where a disk around each 4FGL source is masked to estimate the maximum potential degradation caused by their inclusion.}}
...
\begin{itemize}
    ...
    \item {We assume that contributions from known 4FGL sources are negligible when estimating the DM distribution. While these bright sources can reduce detection efficiency and should ideally be modeled jointly, we estimate the maximum degradation by masking a disk around each 4FGL source. 
...
{Indeed, if there is an excess at about $2\sigma$ significance, which is not accounted by the model, then the model would be excluded at $2 \sigma$ significance compared to a model that can explain the data without excesses, e.g., the data generated from the model itself (cf. figure~\ref{fig:loglike-bootstrap}).}
...
For instance, while not all DM sources were correctly classified due to significant parameter overlap with the Galactic source population, their true class often received a high posterior probability, even if it was not the maximum. This  {shows} that to avoid false negatives when identifying a subdominant class, it is {important} to consider sources with a {reasonably} high posterior probability for that class {even if this probability is not the highest}. 
...
\subsection{{Precision-recall curves for classification of individual DM subhalos}}
\label{app:pr_curve}
