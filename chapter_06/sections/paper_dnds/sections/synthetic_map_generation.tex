\subsection{Synthetic map generation}
...
\noindent
%
A flux map $\mathcal{M}$ will therefore be obtained as the sum of three contributions:
\begin{equation}
    \mathcal{M} = \mathcal{S} + A_\textrm{gal} \cdot \mathcal{G} + \fiso
    \label{eq:dnds:map}
\end{equation}
where $\mathcal{S}$ is the map due to the underlying distribution of sources (obtained from a $\dnds$ model as described in the next subsection), $\mathcal{G}$ is a template map for the foreground emission and $\fiso$ is a constant that denotes the isotropic component discussed above. The constant $A_\textrm{gal}$ is a normalization constant for the galactic foreground template. 

The count map $\mathcal{N}$ is then obtained by multiplying the flux map by the \fermi exposure map $\mathcal{E}$, depicted in Figure \ref{fig:exposure}, and by taking into account the steradian-to-pixel conversion factor. Specifically, the counts in pixel $a$ are obtained as:
\begin{equation}
    \mathcal{N}_a = \mathcal{M}_a \cdot \mathcal{E}_a \cdot \frac{4\pi}{N_\textrm{pix}}
    \label{eq:count}
\end{equation}
%
...
From the count map $\mathcal{N}$, which contains the number of photon counts in each pixel as determined by the model, we finally produce a Poisson realization $\mathcal{N}_\mathcal{P}$, by extracting in each pixel a photon count from a Poisson distribution with mean equal to the photon number of the corresponding pixel in $\mathcal{N}$. $\mathcal{N}_\mathcal{P}$ constitutes our synthetic map realization, which mimics a count map of an experiment with the \fermi specifications, where the underlying model is the one of Eq. \ref{eq:dnds:map}.

In the next Sections we discuss in more detail how the components of Eq. \ref{eq:dnds:map} are modeled.

\subsubsection{Differential source-count distribution}
\label{sec:dnds}


\begin{figure}[t]
\centering
\includegraphics[width=0.49\textwidth]{chapter_06/sections/paper_dnds/img/data_selection/4FGL.pdf}
\includegraphics[width=0.49\textwidth]{chapter_06/sections/paper_dnds/img/data_selection/dNdS_annotato.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[scale=0.60]{chapter_06/sections/paper_dnds/img/data_selection/point_sources.pdf}
\includegraphics[scale=0.60]{chapter_06/sections/paper_dnds/img/data_selection/smoothed_PS.pdf}
...
\item The number $N_i$ of sources in the $i$th flux bin is calculated as $N_i = 4\pi \, (S_{i, \textrm{max}} - S_{i, \textrm{min}}) \, \dnds|_i$ where $\dnds|_i$ is the value at the center of the $i$th bin. The flux bins are determined by subdividing the interval of interest {$[5 \cdot 10^{-12}, 1 \cdot 10^{-7}]$ } cm$^{-2}$ s$^{-1}$ in $10^3$ log-intervals per decade;

\item A Poisson extraction on $N_i$ determines the number of sources $N_{P,i}$ in each flux interval;

\item The sources are randomly positioned on the sphere by uniformly sampling $\cos\theta$ in $[-1,1]$ and $\varphi$ in $[0, 2\pi]$, $\theta$ and $\varphi$ being the spherical coordinates. The selected angular position is then converted into a pixel position in the Healpix pixeling scheme with the \texttt{ang2pix} routine. An example of a point-source map is shown in the upper panel of Figure \ref{fig:point-sources};

\item Finally, convolution with the \fermi PSF shown in Figure \ref{fig:PSF} is applied to each point source. Since sources are assumed to be point-like, we simply replace each point source with an extended circular object with a radial profile equal to the PSF, and whose total flux is equal to the one of the original source. Since the image formation process is linear in the intensity, in case of overlapping sources (either because of their spatial position, or because of the PSF smearing) the resulting flux is additive. An example of flux map with the PSF applied is shown in the lower panel of Figure \ref{fig:point-sources}.
\end{enumerate}
...
\subsubsection{Galactic foreground and isotropic background}
\label{sec:galactic}

\begin{figure}[t]
\centering
\includegraphics[scale=0.6]{chapter_06/sections/paper_dnds/img/data_selection/gf7.pdf}
\caption{\texttt{gll\_iem\_v07} galactic foreground model \cite{Fermi-LAT:2019yla}, integrated in the $(1,10)$ GeV energy bin, in units of photons/(cm$^2$ s sr), at the Healpix resolution $N_\textrm{side} = 128$ ($n=7$).}
...
Confronting with Eq. (\ref{eq:dnds:map}), the $F'_\textrm{iso}$ term denotes the sum of the purely isotropic contribution $F_\textrm{iso}$  and the contribution of sources, $\mathcal{S}$ term, below the 4FGL detection threshold.
...
%
One illustration of a flux map obtained as the sum of all components is shown in Figure \ref{fig:map_with_gf}.
Instead of fixing $A_\textrm{gal}$, in principle, we could also leave it as a free parameter and instruct the network to predict its value. We found, however, that the above procedure constrains $A_\textrm{gal}$ very precisely and it is thus preferable. Furthermore, we also found that fixing $A_\textrm{gal}$ provides a better stability of the neural network.\footnote{Concretely, fixing $A_\textrm{gal}$ reduced the number of simulations required for the training to converge, and the network approached the simulation ground truth faster. When $A_\textrm{gal}$ was instead left free, the uncertainty on its predicted value was substantially larger than that of the Poisson-likelihood fit described above, so fixing it sacrifices little information.}

{  Nonetheless, to better assess the impact of fixing the $A_\textrm{gal}$ value, we also performed a further test using different values in the range $(0.78, 1.0)$ (i.e., a range centered on our fiducial value $0.888 \pm 0.005$, thus encompassing many standard deviations away from its central value.)
...
\begin{figure}[t]
\centering
\includegraphics[scale=0.6]{chapter_06/sections/paper_dnds/img/data_selection/point_sources_fg.pdf}
\caption{The same as in Figure \ref{fig:point-sources}, with the galactic foreground model normalized with $A_\textrm{gal} = 0.886$ and an isotropic background component $F_\textrm{iso} = 4 \cdot 10^{-7}$ cm$^{-2}$ s$^{-1}$ sr$^{-1}$. Units are photons/(cm$^2$ s sr).}
\label{fig:map_with_gf}
\end{figure}

\begin{figure}[h]
\centering
\includegraphics[scale=0.6]{chapter_06/sections/paper_dnds/img/data_selection/counts_map.pdf}
...
\begin{figure}[h]
\centering
\includegraphics[scale=0.6]{chapter_06/sections/paper_dnds/img/data_selection/counts_map_effective.pdf}
\includegraphics[scale=0.6]{chapter_06/sections/paper_dnds/img/data_selection/counts_map_effective_masked.pdf}
\caption{Simulated map in units of counts per pixel after  adding Poisson noise from the synthetic model map of Figure \ref{fig:map-total-counts}, at the same Healpix resolution $N_\textrm{side} = 128$ ($n=7$). The lower panel shows the map with a $|b| < 30^\circ$ latitude cut.}
