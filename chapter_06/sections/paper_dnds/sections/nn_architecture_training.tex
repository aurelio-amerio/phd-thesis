...
On the other hand, the adoption of a flat-image architecture either introduces spherical distortions, due to the mapping of a spherical image on a plane, or may \blue{lose} some of the large scale information contained in the image.
...
{  We stress that with our method we partially \blue{lose} large-scale information and information stored at the border of the patches.
...
\blue{Since the foreground subtraction can leave individual pixels with negative flux values, we do not clip them: a constant offset, chosen so that the minimum pixel value encountered across the training set maps to one, is added to every map before the logarithm is taken. Over-subtracted pixels thus remain encoded at the lower end of the transformed range, and the network retains the information they carry.}
...
In order to correctly estimate the error on a prediction, it is necessary to correctly estimate both the systematic error (sometimes called epistemic error \cite{epistemic-err}) and \blue{aleatoric} error components of a measurement. One way to estimate such components together is through a Bayesian approach, and in particular by defining a Bayesian neural network capable of estimating the posterior distribution of the desired observables.   
...
Following \cite{heteroscedastic-error}, we thus approximate our posterior distribution with a Gaussian distribution, and define the cost function accordingly:
\begin{equation}
    \mathcal{C} = \sum_{i=1}^N  \left[\frac{(y_i^\textrm{true}-y_i^\textrm{pred})^2}{2\sigma_i^2} + \frac{1}{2} \log(\sigma_i^2)  +  \frac{1}{2} \log(2\pi)  \right]
\label{costfunc}    
\end{equation}
...
In Figure \ref{fig:mse-training} we plot the MSE as a function of the training epoch for the training and validation dataset. We can see that, starting from epoch 10, the validation and training lines are close to each other, which means that our model is not overfitting the training dataset. We observe that the MSE reaches a plateau at around epoch 60, meaning that further training of our model would not lead to  improvement in performance.
...
Figure \ref{fig:random-maps} shows a few illustrative examples of $\dnds$, randomly generated according to the priors and parameters intervals listed in Table \ref{tab:prior-dNdS}, together with their reconstructions. 
