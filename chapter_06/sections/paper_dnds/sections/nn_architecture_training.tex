...
On the other hand, the adoption of a flat-image architecture either introduces spherical distortions, due to the mapping of a spherical image on a plane, or may lose some of the large scale information contained in the image.
...
{  We stress that with our method we partially lose large-scale information and information stored at the border of the patches.
...
Since the foreground subtraction can leave individual pixels with negative flux values, we do not clip them: a constant offset, chosen so that the minimum pixel value encountered across the training set maps to one, is added to every map before the logarithm is taken. Over-subtracted pixels thus remain encoded at the lower end of the transformed range, and the network retains the information they carry.
...
In order to correctly estimate the error on a prediction, it is necessary to correctly estimate both the systematic error (sometimes called epistemic error \cite{epistemic-err}) and aleatoric error components of a measurement. One way to estimate such components together is through a Bayesian approach, and in particular by defining a Bayesian neural network capable of estimating the posterior distribution of the desired observables.   
