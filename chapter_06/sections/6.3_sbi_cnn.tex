...
\blue{The configuration adopted for the $dN/dS$ problem discretizes the output into a set of flux bins plus the isotropic background level $F_\mathrm{iso}$; the exact binning, flux range, and training-ensemble size are specified in the paper body (Section~\ref{sec:architechture-and-training}).}
...
The network employs the heteroscedastic Gaussian negative log-likelihood cost function and concrete dropout regularisation---following the Bayesian-neural-network paradigm motivated in Section~\ref{sec:3.3.2}---to estimate both the $dN/dS$ and its uncertainties; the full formalism is presented in the paper body (Section~\ref{sec:bayesian-error}). A frequentist cross-check using the distribution of residuals on held-out validation maps confirms that the Bayesian error bands are well calibrated (Section~\ref{sec:bias}).
