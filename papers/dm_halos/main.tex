...
\newcommand{\DMsubhaloUnas}{\cite{2010PhRvD..82f3501B,2012A&A...538A..93Z,2012ApJ...747..121A,2012JCAP...11..050Z,2012PhRvD..86d3504B,2014GrCo...20...47B,2014PhRvD..89a6014B,2015JCAP...12..035B,2016JCAP...05..028S,2016ApJ...825...69M,2017JCAP...04..018H,2017PhRvD..96f3009C,2019Galax...7...90C,2019JCAP...11..045C,2019JCAP...07..020C,2022PhRvD.105h3006C,2023JCAP...07..033B,2023MNRAS.520.1348G}}
...
\cite{Fermi-LAT:2011sla,2016ApJ...825...69M,2016ApJ...820....8S,Lefaucheur:2017ajt,2019JCAP...11..045C,2020MNRAS.492.5377L,2020MNRAS.495.1093H,2021RAA....21...15Z,2021MNRAS.505.5853G,Bhat:2022,2023JCAP...07..033B,2023MNRAS.521.6195M,2023RASTI...2..735M,2023MNRAS.520.1348G}}
...
The latest \Fermi-LAT catalog (4FGL-DR4)~\cite{Fermi-LAT:2022byn,Ballet:2023qzs} contains 2428 unassociated sources (unIDs), which is about one third of all detected sources.
...
In this work we use the 4FGL-DR4 catalog~\cite{Fermi-LAT:2022byn,Ballet:2023qzs}. We group the known types of associated sources into two classes:%
\footnote{Cf. table 5 in ref.~\cite{Fermi-LAT:2022byn} for details about the types of sources. Galactic sources: psr, hmb, sfr, snr, pwn, gc, gal, bin, msp, lmb, spp, glc, nov. Extragalactic sources: bll, sbg, rdg, css, bcu, ssrq, fsrq, sey, nlsy1, agn.}
\begin{itemize}
    \item Galactic \new{(dominated by millisecond pulsars at high latitudes~\cite{smith2023third,Fermi-LAT:2019yla,DGRB-review,Hooper:2024,Cholis:2014noa,Caraveo:2013lra})}: pulsars, binaries, star-forming regions, supernova remnants, pulsar wind nebulae, globular clusters, and novae;
    \item Extragalactic (\new{mostly AGNs~\cite{fossati1998unifying,Fermi-LAT:2019yla,DGRB-review,Dermer:2009zz,Fermi-LAT:2014ryh,Ajello:2015mfa,Lisanti:2016jub}}): blazars, quasars, starburst galaxies, normal galaxies, radio galaxies and radio sources, Seyfert galaxies, and non-blazar active galaxies.
\end{itemize}
%
%% aure: working here
%
...
\end{equation}
where $dN_\gamma/dE$ is the theoretical spectrum of gamma rays produced in an annihilation of a pair of DM particles from ref.~\cite{Arina:2023eic}. 
\item
...
We use the same Gaussian priors for the $\beta$ parameter as in the 4FGL-DR4 catalog with mean 0.1 and sigma 0.3~\cite{Ballet:2023qzs}.
...
\begin{figure}[t]
    ...
    \panel{Right panel}: Class probabilities for unassociated 4FGL-DR4 sources (for $\rmfamily E_0 = 1 \,GeV$), on top of the mixture distribution (same as the middle panel). The color scale is proportional to the class probabilities, according to the formula $\textrm{color}(x_i)=\textrm{red}\cdot p(C_k= \textrm{gal}|x_i) + \textrm{blue}\cdot p(C_k=\textrm{ext}|x_i)$, e.g., a purple-colored source is a source for which the posterior class probabilities are similar and the classification is ambiguous. 
    }
    \label{fig:mixtures}
\end{figure}
...
\toprule
type & $\rmfamily N_{ext}$ & $\rmfamily N_{Gal}$ & \% Ext & \% Gal & $\rmfamily N_{tot}$\\
\midrule
...
\toprule
 ...
 $\mdm$ & \% DM & $N_\textrm{subhalo}^\textrm{ROI}$  & $N_\textrm{subhalo}^\textrm{total}$ & $\langle \sigma v \rangle$ & $\Delta \textrm{TS}_0$ & $N_\textrm{subhalo}^\textrm{ROI}$ & $N_\textrm{subhalo}^\textrm{total}$ & $\langle \sigma v \rangle$ \\
  $\rmfamily [GeV]$ & & & & [$\rmfamily cm^3 \, s^{-1}$] & & & & [$\rmfamily cm^3 \, s^{-1}$] \\
\midrule
...
We take the parameters $\bfseries x$ of the log-parabola fit as the input features. 
We then
...
We specify the spectral energy distribution (SED), $dN_\gamma/dE$, using the recent estimations from CosmiXs \cite{Arina:2023eic}.
...
\begin{enumerate}
    \item Initialize $\{\btheta_k\}=\{\btheta_k\}_\textrm{old}$, and $\boldsymbol{\pi} = \boldsymbol{\pi}_\textrm{old}$;
    \item Evaluate $p(\textbf{Z}|\textbf{X},\{\btheta_k\}_\textrm{old},\boldsymbol{\pi}_\textrm{old})$;
    ...
    \begin{equation}
     ...
     \sum_\textbf{Z} 
    p(\textbf{Z}|\bx,\{\btheta_k\}_\textrm{old},\boldsymbol{\pi}_\textrm{old})~
\ln p(\textbf{X,Z}|\{\btheta_k\},\boldsymbol{\pi})~.  
    \end{equation}
    \item Update the parameters as:
    \begin{equation}
      \{\btheta_k\}_\textrm{new},\boldsymbol{\pi}_\textrm{new} =
      \underset{\{\btheta_k\},\boldsymbol{\pi}}
      \textrm{argmax}~Q(\{\btheta_k\},\boldsymbol{\pi})
    \end{equation}
...
\begin{equation}
    (\pi_k)_\textrm{new} = \frac{1}{N}\sum_i^N \gamma(z_{ik})~,
\end{equation}
...
\begin{figure}[t]
    ...
    \caption{Distribution of log-likelihood for the ``$p+c$'' model with astro-only components for 1000 datasets simulated from the best-fit ``$p+c$'' model for $\rmfamily E_0=100 \, \textrm{MeV}$ (\panel{left panel}) and $\rmfamily E_0=1 \, \textrm{GeV}$ (\panel{right panel}). The red line represents the log-likelihood of the ``$p+c$'' model with astro-only components for the unassociated 4FGL-DR4 sources.}
...
\begin{figure}[t]
    ...
    \caption{Predicted source classes for unassociated 4FGL-DR4 sources according to the posterior class probabilities for the $\rmfamily E_0 = 1 \,GeV$ case.
...
\begin{figure}[t]
    ...
    \panel{Right panel}: markers colored according to the predicted class, given by $C_i^{pred} = \textrm{argmax}_k \{ \tilde{p}(z_{ik}=1|\mathbf{x}_i) \}$.
    }
