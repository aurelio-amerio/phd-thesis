...
\item
We keep DM subhalos detected with significance TS~$>25$, where $\textrm{TS}=-2\ln {\cal L}/{\cal L}_*$ is the likelihood-ratio test statistic between the models with and without the subhalo. Such a threshold corresponds to the significance for PS detection in the {\Fermi}-LAT catalogs.
...
\be
\label{eq:L}
{\cal L}(\langle\sigma v\rangle, \pi_k, \btheta_\textrm{cov}) = \prod_{i=1}^{N^\textrm{ROI}_\textrm{unas}} \tilde{p}_\textrm{unas}(\bx_i|\sigmav,\pi_k, \btheta_\textrm{cov};\mdm,\textrm{ch.}).
\ee
...
\be
\textrm{TS}(\sigmav) = -2\ln \frac{{\cal L}(\sigmav)}
{{\cal L}_\textrm{max}}, 
\label{eq:TS}
\ee
where ${\cal L}(\sigmav)$ is the likelihood optimized over the rest of parameters $\pi_k$ and $\btheta_\textrm{cov}$, and ${\cal L}_\textrm{max}={\cal L}(\sigmav^*,\pi_k^*,\btheta_\textrm{cov}^*)$ is the value of the likelihood for the best-fit parameters. 
