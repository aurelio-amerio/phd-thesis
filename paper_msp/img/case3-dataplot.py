...
infile = open('../RUN2/forfiglum.out', 'r')
for line in infile.readlines():
        params = line.split()
        
        gamma +=  [float(params[0])]
        lum += [float(params[1])]  
        lumupper += [float(params[2])]  
        lumlower += [float(params[3])]  
        lumerrcen += [float(params[3])] 
        lumerror += [float(params[3])]
        gammaerrcen += [float(params[3])]
        gammaerror  += [float(params[3])]
...
infile = open('bandsforplot-case3.out', 'r')
for line in infile.readlines():
        params = line.split()
...
plt.text(3.83E3, 8.0E35, r'$\langle N_{\rm MSP} \rangle = (3.6\times 10^{-5)}\, L_V$',rotation=0,color='black',fontsize=17)



plt.tick_params(which='both',labelsize=14)

plt.xlabel(r'{\rm Visible}\,{\rm Luminosity}\, $(L_{\odot})$', fontsize=18) ##set the xlabel for the plot
...
plt.axis([3.0E3, 3.0E6, 1.0E31, 3.0E36])
plt.yscale('log')
plt.xscale('log')
...
fig.savefig('case3-dataplot.pdf')
