...
We model the spectral energy distribution (SED) of these sources as power-laws with index $\varGamma$ and exponential cutoffs $E_{\rm cut}$, whose specific parameter values are reported in \cref{table:BLLACS}, together with the minimum and maximum luminosities of the population.
...
Astrophysical sources, on the other hand, have a peak in emission at redshifts $z\sim 0.3$--$0.4$ at $E\sim {50}\,{\rm GeV}$, and $z\sim 0.1$--$0.2$ at $E\gtrsim 1\,{\rm TeV}$
as shown {in Fig. \ref{fig:window_main} and in Fig. \ref{fig:zWindows}} in {\cref{app:model}}.
The latter range is a consequence of strong absorption of $\gamma$-rays above $\sim 1 {\rm TeV}$ due to interactions with the extragalactic background light (EBL), which limits the $\gamma$-ray horizon to $z\sim 0.1$--$0.2$ independently of the type of source.
...
\begin{multline}
    (\varDelta C^{\gamma g}_\ell)^2 \,(2\,\ell+1)\,f_{\rm sky} =  \\ (C_\ell^{\gamma g})^2 + \left[C_\ell^{\gamma \gamma} + \frac{C_N}{(B_\ell)^2}\right]\,\left(C_\ell^{gg} + C_{N_g} \right)\;, \label{eq:Delta}
\end{multline}
...
\be
(\varDelta C_\ell^{\gamma \gamma})^2 = \frac{2}{(2\,\ell+1)\,f_{\rm sky}} \left[C_\ell^{\gamma \gamma} + \frac{C_N}{(B_\ell)^2} \right]^2\;.\label{eq:variance_gammagamma}
\ee
...
\hline
  & $\varGamma$ & $E_{\text{cut}}$ [GeV] & $L_{\rm min}$ [erg/s] & $L_{\rm max}$ [erg/s]  \\ [0.5ex]
 \hline
