...
We stress, however, that only the energy spectrum is a genuine prediction: once the \WIMP mass and annihilation channel are fixed, the spectral shape follows.
The morphology is instead a measurement, fitted with a halo profile that enters as an independent astrophysical input; in principle, any \WIMP candidate could be paired with any density profile, so the fit alone carries little weight.
The agreement is meaningful because the profile the data require---cuspy, with inner slope $\gamma \approx 1.1$--$1.3$, and approximately spherical---is the kind that N-body simulations of cold dark matter structure formation produce on their own~\cite{Navarro:1996gj,Springel:2008cc}.
The steepening beyond $\gamma = 1$ is qualitatively consistent with adiabatic contraction, as already noted in Section~\ref{sec:4.1.3}.
The dark matter model thus describes the morphology rather than predicting it: the measured shape falls within the range expected for cold dark matter halos, and a single normalization fixes the observed flux.
