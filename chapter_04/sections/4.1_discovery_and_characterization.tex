...
\blue{We stress, however, that only the spectrum is genuinely predicted: the morphology is measured, and fitted with a halo profile that is an independent astrophysical input.}
\blue{In principle, any WIMP candidate could be paired with any density profile; the nontrivial point is that the profile the fit requires---steep ($\gamma \approx 1.1$--$1.3$) and approximately spherical---was not invented for the occasion.}
\blue{N-body simulations of cold dark matter structure formation independently produce approximately spherical inner halos with cuspy density profiles~\cite{Navarro:1996gj,Springel:2008cc}, and the steepening beyond $\gamma = 1$ is qualitatively consistent with adiabatic contraction, as already noted in Section~\ref{sec:4.1.3}.}
\blue{What the model achieves is therefore an economy of description, not a prediction: the measured morphology falls within the range that cold dark matter halos are expected to occupy, and a single normalization accounts for the observed flux.}
