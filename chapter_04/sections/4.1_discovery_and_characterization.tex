...
The \blue{explored} halo profile was a Navarro--Frenk--White distribution, \blue{which once fit displayed} a mildly steepened inner slope, $\rho(r) \propto r^{-\gamma}$ with $\gamma = 1.1$, consistent with the adiabatic contraction expected from baryonic dynamics in the inner Galaxy (see Section~\ref{sec:density_profiles}).
...
\blue{First, the spectral shape of the excess was relatively stable regardless of the adopted diffuse model, a robustness that we quantify together with the other established properties of the signal in Section~\ref{sec:4.1.3}.}
...
\blue{The data strongly disfavor any elongation along or perpendicular to the Galactic Plane by more than $\sim 20\%$, and the centroid of the emission lies within $\sim 0.05^\circ$ of the position of Sgr~A$^*$~\cite{Daylan:2014rsa}.}
...
\blue{This minimal model can accommodate all three observables simultaneously.}
...
\blue{The GCE energy spectrum (Section~\ref{sec:4.1.2}) and its spatial morphology (Section~\ref{sec:4.1.3}) are independently consistent with annihilation of a $\sim 30$--$50$~GeV WIMP into $b\bar{b}$ final states, distributed according to a contracted NFW profile with inner slope $\gamma \approx 1.1$--$1.3$.
...
\blue{A more robust independent test of the annihilation hypothesis comes from the Milky Way dwarf spheroidal galaxies, which are dark-matter-dominated and nearly free of astrophysical gamma-ray backgrounds.
...
\blue{The comparison is not decisive, however, because it inherits large systematics on both sides.
...
% \blue{Consistency, moreover, is not uniqueness: as we discuss in the following sections, models in which the excess traces the stellar bulge of the inner Galaxy have been claimed to describe the same data equally well or better (cf.\ Sections~\ref{sec:4.2.1} and~\ref{sec:4.3.2}).}