...
This section traces the observational history of the \GCE, from its first identification in early Fermi-LAT data through the systematic studies that refined its spectral and morphological properties.
...
The explored halo profile was a \NFW distribution, which once fit displayed a mildly steepened inner slope, $\rho(r) \propto r^{-\gamma}$ with $\gamma = 1.1$, consistent with the adiabatic contraction expected from baryonic dynamics in the inner Galaxy (see Section~\ref{sec:density_profiles}).
...
By selecting only the top two quartiles of front-converting Ultraclean events ranked by this parameter, they suppressed the tails of the \gls{PSF_} substantially, particularly at low energies where the \PSF is broadest.
...
Because such a broken power-law can readily parameterize astrophysical emission --- from unresolved \glspl{MSP_}, for instance --- this analysis made clear that spectral information alone could not distinguish between dark matter and astrophysical origins.
...
The spatial morphology of the excess is approximately spherically symmetric.\footnote{\blue{The qualifier ``approximately'' carries much of the interpretive weight. An exactly spherical, truly diffuse emission is what dark matter annihilation predicts; a morphology tracing the stellar bulge, possibly built from unresolved \glspl{MSP_}, would point to an astrophysical origin. We return to this question in Sections~\ref{sec:4.2} and~\ref{sec:4.3}.}}
% Daylan et al.
...
The observational properties established above align closely with the predictions of \gls{WIMP_} dark matter annihilation, a coincidence that has sustained interest in this interpretation for over a decade.
...
Because annihilation occurs essentially at rest\footnote{\Glspl{WIMP_} are a cold dark matter candidate and are therefore non-relativistic.}, the observed spectrum is fixed by the mass and annihilation channel, while the angular distribution on the sky is determined entirely by the density profile.
...
\blue{We stress, however, that only the energy spectrum is a genuine prediction: once the \WIMP mass and annihilation channel are fixed, the spectral shape follows.}
\blue{The morphology is instead a measurement, fitted with a halo profile that enters as an independent astrophysical input; in principle, any \WIMP candidate could be paired with any density profile, so the fit alone carries little weight.}
\blue{The agreement is meaningful because the profile the data require---cuspy, with inner slope $\gamma \approx 1.1$--$1.3$, and approximately spherical---is the kind that N-body simulations of cold dark matter structure formation produce on their own~\cite{Navarro:1996gj,Springel:2008cc}.}
\blue{The steepening beyond $\gamma = 1$ is qualitatively consistent with adiabatic contraction, as already noted in Section~\ref{sec:4.1.3}.}
\blue{The dark matter model thus describes the morphology rather than predicting it: the measured shape falls within the range expected for cold dark matter halos, and a single normalization fixes the observed flux.}
