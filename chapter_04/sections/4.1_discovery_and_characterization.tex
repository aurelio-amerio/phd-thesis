...
\blue{The fact that three independent observables---spectrum, morphology, and flux normalization---can be described by a simple, internally consistent dark matter model with parameters close to the thermal relic prediction has sustained interest in this interpretation for over a decade, and has kept the GCE among the leading candidate signals in indirect detection.}
...
\blue{Consistency, moreover, is not uniqueness: as we discuss in the following sections, analyses in which the excess traces the stellar bulge of the inner Galaxy have been claimed to describe the same data equally well or better (cf.\ Sections~\ref{sec:4.2.1} and~\ref{sec:4.3.2}).}
