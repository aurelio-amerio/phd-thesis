...
The analyses discussed in the preceding sections suffer from a shared weakness: they all operate directly within the complex gamma-ray environment of the Galactic Center, where the results depend sensitively on the diffuse-emission systematics discussed in Section~\ref{sec:4.3}.
...
The \MSP hypothesis requires the pulsars responsible for the excess to be systematically fainter than those observed in any other environment, since otherwise Fermi-LAT should have already resolved many of them individually (cf.\ Section~\ref{sec:4.2.2}).
...
The luminosity function we obtain is consistent with that measured independently for \glspl{MSP_} in the Galactic plane~\cite{Holst:2024fvb}:
...
Only three \MSP candidates have been identified in this region~\cite{Holst:2024fvb}, and targeted radio follow-up campaigns of unassociated Fermi sources have not so far uncovered a hidden population.
...
This convergence should be weighed with the caveats of Section~\ref{sec:4.3.3} in mind: the \SCD result remains provisional, although the globular-cluster constraint does not depend on the diffuse-model systematics that drive those caveats.
