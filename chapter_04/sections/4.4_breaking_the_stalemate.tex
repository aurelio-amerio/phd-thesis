...
The analyses discussed in the preceding sections \blue{suffer from a shared weakness}: they all operate directly within the complex gamma-ray environment of the Galactic Center, where the results depend sensitively on the diffuse-emission systematics discussed in Section~\ref{sec:4.3}.
...
Whether one uses template fitting, non-Poissonian statistics, or neural network-based inference, every conclusion about the nature of the \GCE inherits these uncertainties.
...
The \MSP hypothesis requires the pulsars responsible for the excess to be systematically fainter than those observed \blue{in any other environment}, since otherwise Fermi-LAT should have already resolved many of them individually (cf.\ Section~\ref{sec:4.2.2}).
Evolutionary models have been used to argue for a fainter bulge population on these grounds~\cite{Fragione:2017rsp,Ploeg:2020jeh,Ye:2022yxt}.
How much fainter the inner-Galaxy pulsars could be in practice, however, and the actual age distribution of \glspl{MSP_} in the bulge remain open questions.
...
Building on an earlier study by Hooper and Linden~\cite{Hooper:2016rap}, in Amerio, Hooper, and Linden~\cite{Amerio:2024qor} we measured the gamma-ray luminosity function of \glspl{MSP_} across the Milky Way's globular cluster system, using the full Fermi-LAT dataset to date.
The luminosity function we obtain is consistent with that measured independently for \glspl{MSP_} \blue{in the Galactic plane}~\cite{Holst:2024fvb}:
although the age distribution of the Inner Galaxy pulsars could in principle differ from that of globular clusters, the two environments seem to empirically yield similar luminosity functions.

If the pulsars responsible for the excess indeed share this luminosity function, the implications for the \GCE are direct: Fermi-LAT's source catalogs should already contain $N_\mathrm{MSP} \sim 17{-}37$ individually resolved pulsars from the Inner Galaxy population.
Only three \MSP candidates have been identified in this region~\cite{Holst:2024fvb}, and \blue{targeted radio follow-up campaigns of unassociated Fermi sources} have not so far uncovered a hidden population.
...
An \MSP origin for the \GCE would therefore require a pulsar population unlike any observed so far --- a requirement for which the hypothesis, as currently formulated, offers no explanation.

This independent constraint connects directly to the recent findings of List et al.~\cite{List:2025qbx}.
As discussed in Section~\ref{sec:4.3.3}, their energy-dependent \CNN analysis pushes the \SCD of any \GCE point sources to extremely faint levels, requiring on the order of $2 \times 10^5$ sources --- each producing far less than one detected photon --- to account for the excess.
...
% However, the globular cluster results undermine the leading mechanism proposed to explain such a population: if old age does not produce ultra-faint MSPs, it is unclear what would generate $\mathcal{O}(10^5)$ sources of the necessary faintness in the Inner Galaxy.
...
\blue{This convergence should be weighed with the caveats of Section~\ref{sec:4.3.3} in mind: the \SCD result remains provisional, although the globular-cluster constraint does not depend on the diffuse-model systematics that drive those caveats.}
The remainder of this chapter is based on \cite{Amerio:2024qor} and presents the analysis in full.
