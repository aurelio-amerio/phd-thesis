...
\blue{By the late 2010s, the NPTF photon statistics, wavelet power, and stellar bulge morphology appeared to have settled the question in favor of the MSP interpretation.}
\blue{As we will see in Section~\ref{sec:4.3}, however, subsequent studies exposed weaknesses in each of these analyses, casting doubt on their conclusions and setting off a series of rebuttals and refinements.}
...
\blue{These constraints rest on our current understanding of MSP formation and evolution, in the Galactic center and elsewhere, rather than on the GCE data itself; revisions to that understanding could weaken them.}
...
\blue{This discrepancy implies that the GCE can originate from MSPs only if those pulsars are systematically fainter than any MSP population observed so far.}
\blue{Such dimming is not implausible: unlike the disk and globular cluster populations, which are continuously replenished with freshly recycled pulsars, a bulge population that stopped forming new MSPs long ago would have faded through spin-down.}
\blue{Evolutionary models have indeed been used to argue for a fainter bulge population on these grounds~\cite{Fragione:2017rsp,Ploeg:2020jeh,Ye:2022yxt}.}
\blue{How much fainter the inner-Galaxy pulsars could realistically be, however, remains an open question.}
This ``missing bright pulsars'' problem \blue{is} one of the strongest empirical challenges to the MSP hypothesis.
...
\blue{In Section~\ref{sec:msp_paper}, we present a dedicated analysis based on~\cite{Amerio:2024qor} that updates and extends this line of argument.}