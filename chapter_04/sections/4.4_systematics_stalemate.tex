% Section 4.4: The Systematics Stalemate
...
The evidence for point sources presented in Section~\ref{sec:4.3.2} rested largely on the Non-Poissonian Template Fitting (NPTF) framework, which exploits the difference in photon statistics between smooth (Poissonian) emission and populations of unresolved point sources.
...
Buschmann et al.~\cite{Buschmann:2020adf}, with similar conclusions reached independently by Calore et al.~\cite{Calore:2021jvg}, argued that the signal injection failures reported by Leane and Slatyer were driven primarily by the use of the older \texttt{p6v11} Galactic diffuse emission model, which suffers from severe over-subtraction in the inner Galaxy.
...
As discussed in Section~\ref{sec:4.2.3}, the excess was initially characterized as approximately spherically symmetric, centered on Sgr~A$^*$, and consistent with a generalized NFW profile with inner slope $\gamma \approx 1.1$--$1.3$.
...
However, a series of analyses beginning with Macias et al.~\cite{Macias:2016nev,Macias:2019omb} and Bartels et al.~\cite{Bartels:2018xom} challenged this picture by arguing that the GCE traces the boxy, X-shaped stellar bulge of the Milky Way rather than a smooth, spherically symmetric halo.
...
Whether the GCE prefers an NFW or bulge morphology can shift depending on the interstellar emission model adopted, the masking procedure applied to the Galactic plane and known point sources, and how the Fermi Bubbles emission is treated at low latitudes~\cite{Cirelli:2024ssz}.
...
This spectral argument operates independently of the morphological debate and has been cited as evidence against a complete MSP explanation.
