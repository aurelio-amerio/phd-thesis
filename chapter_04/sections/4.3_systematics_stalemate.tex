...
The evidence reviewed in the preceding section, both for and against the \MSP hypothesis, was largely obtained through template-based analyses that impose a fixed spatial morphology on each emission component.
These analyses share a common vulnerability: their conclusions depend on modeling choices for which no unique, model-independent prescription exists.
...
One approach relaxes the rigid-morphology assumption itself.
\emph{Adaptive-template} methods such as skyFACT~\cite{Storm:2017arh}, employed by Bartels et al.~\cite{Bartels:2017vsx} (cf.\ Section~\ref{sec:4.2.1}), let the data re-modulate the input templates through $\sim 10^5$ pixel-level nuisance parameters in a penalized likelihood.
By construction, this mitigates the morphology mis-specification problem.
This flexibility trades one set of choices for another: how strongly the regularization constrains the re-modulation, and the fact that the fit remains anchored to the input templates it adjusts.
It also leaves untouched the underlying uncertainty in the interstellar emission model.
...
A recent work by the same group~\cite{Manconi:2025ogr} further enhanced the analysis with an improved estimation of the Galactic Diffuse Emission adopting the skyFACT framework, further strengthening the evidence for point sources in the \GCE.
...
However, the stellar-bulge analyses of Macias et al.~\cite{Macias:2016nev,Macias:2019omb} and Bartels et al.~\cite{Bartels:2017vsx}, introduced in Section~\ref{sec:4.2.1}, challenged this picture by finding that the boxy, X-shaped stellar bulge fits the \GCE better than a spherically symmetric \NFW halo.
...
Because the true diffuse background in the inner Galaxy is not known to the precision required, the morphological fit is inherently subject to systematic uncertainties that propagate directly into the inferred spatial preference.
...
Beyond the morphological debate, which remains unresolved at the spatial level, an independent spectral observation initially appeared to complicate the astrophysical interpretation.
...
The presence of a power-law tail at higher energies is difficult to reconcile with prompt magnetospheric emission alone.
...
Direct analyses of the high-energy emission support this reading.
...
While \cite{Manconi:2025ogr} argued that including energy information does not substantially alter the conclusions of the \gls{1pPDF_}, List et al.~\cite{List:2025qbx} found that it has a strong effect on the inferred \SCD and the fraction of emission that can be attributed to point sources.
...
The same standard of scrutiny we applied to the \NPTF must apply here, because the analysis rests on approximations of its own.
...
Should the result survive this scrutiny, the implications would be twofold.
