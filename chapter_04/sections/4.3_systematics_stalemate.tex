...
The evidence reviewed in the preceding section, both for and against the \MSP hypothesis, was \blue{largely} obtained through template-based analyses \blue{that impose a fixed spatial morphology on each emission component}.
\blue{These analyses} share a common vulnerability: their conclusions depend on modeling choices for which no unique, model-independent prescription exists.
...
\blue{One approach relaxes the rigid-morphology assumption itself.}
\blue{\emph{Adaptive-template} methods such as skyFACT~\cite{Storm:2017arh}, employed by Bartels et al.~\cite{Bartels:2017vsx} (cf.\ Section~\ref{sec:4.2.1}), let the data re-modulate the input templates through $\sim 10^5$ pixel-level nuisance parameters in a penalized likelihood.}
\blue{By construction, this mitigates the morphology mis-specification problem.}
\blue{This flexibility trades one set of choices for another: how strongly the regularization constrains the re-modulation, and the fact that the fit remains anchored to the input templates it adjusts.}
\blue{It also leaves untouched the underlying uncertainty in the interstellar emission model.}
...
\subsection{Challenges to NPTF}
\label{sec:4.3.1}

The evidence for point sources presented in Section~\ref{sec:4.2.1} rested largely on the \NPTF framework, which exploits the difference in photon statistics between smooth (Poissonian) emission and populations of unresolved point sources.
...
Similar conclusions were reached independently by Calore et al.~\cite{Calore_2021} using the \gls{1pPDF_} technique. \Gls{1pPDF_} is an analytical framework mathematically equivalent in its goals to the \NPTF, leveraging pixel-count statistics to reconstruct the \SCD. By combining \gls{1pPDF_} with adaptive template fitting, they demonstrated that point sources constitute a robust sub-threshold component of the \GCE, provided the diffuse emission is accurately modeled.
A recent work by the same group~\cite{Manconi:2025ogr} further enhanced the analysis with an improved estimation of the Galactic Diffuse Emission adopting the \blue{skyFACT} framework, further strengthening the evidence for point sources in the \GCE.

Ultimately, this exchange exposed an underlying tension: results from analysis based on \NPTF-like methods depend sensitively on analysis choices that remain a matter of debate.
...
% The preference for point sources can be made to appear or vanish depending on the diffuse model, the treatment of spatial asymmetries, and the other modeling choices enumerated at the opening of this section.
% No analysis to date has achieved a resolution that all groups accept, and the community consensus has shifted accordingly: the NPTF, as currently implemented, cannot definitively distinguish between smooth and point-source emission in the inner Galaxy.
No analysis to date has achieved a resolution that all groups accept. The \NPTF---and specifically the \gls{1pPDF_} technique---remain valid statistical techniques; what part of the community questions is whether, given the sensitivity to the modeling choices discussed above, the conclusions reached so far on smooth versus point-source emission in the inner Galaxy can be regarded as definitive.
...
However, the stellar-bulge \blue{analyses} of Macias et al.~\cite{Macias:2016nev,Macias:2019omb} and Bartels et al.~\cite{Bartels:2017vsx}, introduced in Section~\ref{sec:4.2.1}, challenged this picture by finding that the boxy, X-shaped stellar bulge fits the \GCE better than a spherically symmetric \NFW halo.
...
Whether the \GCE prefers an \NFW or bulge morphology can shift depending on the same diffuse-emission systematics discussed at the opening of this section~\cite{Macias:2019omb}.
Because the true diffuse background in the inner Galaxy is not known to the precision required, \blue{the morphological fit} is inherently subject to systematic uncertainties that propagate directly into the \blue{inferred spatial preference}.
...
Beyond the morphological debate, which remains unresolved at the spatial level, an independent spectral observation \blue{initially appeared to complicate} the astrophysical interpretation.
Cholis et al.~\cite{Cholis:2021rpp} examined the high-energy behavior of the \GCE spectrum above $\sim 10$~GeV and found emission extending well beyond the energies at which \glspl{MSP_} are expected to contribute.
The gamma-ray spectra of known \glspl{MSP_} exhibit exponential cutoffs at a few GeV, a characteristic feature of curvature radiation in pulsar magnetospheres.
\blue{The presence of a power-law tail at higher energies is difficult to reconcile with prompt magnetospheric emission alone.
It does not, however, rule out an \MSP origin: the electrons and positrons injected by a bulge \MSP population would up-scatter ambient photons from the interstellar radiation fields of the inner Galaxy.
This inverse Compton component can naturally account for the high-energy tail~\cite{Song:2019nrx,manconi2024galacticcenterexcesshighest}.
The tail therefore constrains the composition of the \MSP emission rather than excluding an \MSP origin.}

\blue{Direct analyses of the high-energy emission support this reading.}
Manconi et al.~\cite{manconi2024galacticcenterexcesshighest} extended the \gls{1pPDF_} technique to the $>10$~GeV regime, combining it with adaptive template fitting to evaluate both the morphology and the photon-count statistics of the high-energy tail.
...
Crucially, their \gls{1pPDF_} analysis reconstructed a significant population of faint, sub-threshold point sources at these high energies.
...
Manconi et al.\ interpret this as evidence that the high-energy tail of the \GCE can be explained by a population of point sources---possibly \glspl{MSP_} with harder spectra or a distinct sub-population---without requiring a dark matter contribution or an exotic secondary component.
...
% It is precisely this impasse that motivates the energy-dependent inference discussed in Section~\ref{sec:4.3.3} and the independent constraints, obtained far from the Galactic Center, that we develop in Section~\ref{sec:4.4}.

\subsection{Recent Revival of the Dark Matter Hypothesis}
\label{sec:4.3.3}
While the previous sections would seem to suggest that the \GCE is now firmly established as a population of unresolved point sources, the situation is more nuanced.
A recent analysis has reframed the question entirely by incorporating energy information into the point-source inference for the first time.
While \cite{Manconi:2025ogr} argued that including energy information does not substantially alter the conclusions of the \gls{1pPDF_}, List et al.~\cite{List:2025qbx} found that it has a \blue{strong} effect on the inferred \SCD and the fraction of emission that can be attributed to point sources.
List et al.~\cite{List:2025qbx} developed a \gls{CNN_}-based \gls{SBI_} framework that simultaneously fits the spatial and spectral properties of the inner Galaxy across ten logarithmically spaced energy bins from 2 to 20~GeV.
Unlike current iterations of the \gls{1pPDF_} technique, which discarded all spectral information and neglected pixel-to-pixel correlations from the finite \PSF, this approach exploits the fact that the \GCE has a distinct energy spectrum from the dominant diffuse backgrounds, providing an additional lever arm to separate signal from astrophysical contamination (see Chapter~\ref{ch:methods} for background on \SBI).

The inclusion of energy information substantially alters the inferred \SCD.
...
The same energy-integrated picture emerges from the pixel-count analyses of Section~\ref{sec:4.3.1}: the \SCD reconstructed with the \gls{1pPDF_} by Calore et al.~\cite{Calore_2021} is comparable to the energy-independent distributions shown in Figure~\ref{fig:gce_scd}. Manconi et al.~\cite{Manconi:2025ogr} likewise report that their updated measurement remains compatible with that earlier result.
The energy-dependent \CNN drives the \SCD to far fainter fluxes: the inferred distribution resides almost entirely below the one-photon threshold, meaning that a typical source would not even produce a single detected photon, see Figure~\ref{fig:gce_scd}.
...
\begin{figure}[t]
	...
	\caption{\Gls{SCD_} of the \GCE inferred via three approaches.
...
This result has a direct consequence.
...
This consistency reopens the dark matter hypothesis, but it does not demonstrate it: the result removes an obstacle to a smooth-emission origin rather than establishing one.

\blue{The same standard of scrutiny we applied to the \NPTF must apply here, because the analysis rests on approximations of its own.
The astrophysical components in the training simulations are built from the same standard diffuse templates that have repeatedly been shown to mis-model the inner Galaxy, so the inference inherits the diffuse-emission systematics discussed at the opening of this section.
This dependence is not hypothetical.
In an appendix (their Fig.~S12), List et al.\ retrained their network with an alternative Galactic diffuse model; the recovered \SCD shifted almost an order of magnitude brighter, roughly consistent with the earlier energy-independent results~\cite{List:2020mzd}.
The authors themselves concede that the result ``is not independent of the background diffuse model''~\cite{List:2025qbx}.
The counterweight is that the alternative model fits the data far worse than their baseline ($2\Delta\ln\mathcal{L} \approx 648$), and List et al.\ frame the exercise as a stress test against mis-modeling rather than as an equally valid background choice.
A third approximation is subtler: the simulator assigns a single shared intrinsic spectrum to all point sources, whereas the Galactic Center hosts several source classes with distinct spectra.
This is itself a form of forced mis-modeling, and it can bias the inference.
Whether the result survives independent reanalysis with different background treatments therefore remains to be seen; until then, it should be read as a demonstration of what spectral information can bring to this problem, not as a settled measurement.}

\blue{Should the result survive this scrutiny}, the implications would be twofold.
