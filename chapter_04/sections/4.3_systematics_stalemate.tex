...
The exchange exposed \blue{an underlying tension}: the NPTF results depend sensitively on analysis choices that remain \blue{a matter of debate}.
...
No analysis to date has achieved a resolution that all groups accept. \blue{The NPTF itself remains a valid statistical technique; what part of the community questions is whether, given the sensitivity to the modeling choices discussed above, the conclusions reached so far on smooth versus point-source emission in the inner Galaxy can be regarded as definitive.}
