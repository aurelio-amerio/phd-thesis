...
The key observation is that this minimal signal model succeeds on all three fronts.
As established in Section~\ref{sec:4.1}, the GCE spectrum, morphology, and spatial extent are each independently consistent with annihilation of a $\sim 30$--$50$~GeV WIMP into $b\bar{b}$ final states, distributed according to a contracted NFW profile with inner slope $\gamma \approx 1.1$--$1.3$ (Section~\ref{sec:4.1.2} for the spectral fits; Section~\ref{sec:4.1.3} for the morphological characterization).
