% Section 4.2: Competing Interpretations
...
Because annihilation occurs essentially at rest\footnote{WIMPS are a cold dark matter candidate, as such are non-relativistic}, the observed spectrum is fixed by the mass and annihilation channel, while the angular distribution on the sky is determined entirely by the density profile. \aure{maybe elaborate more on this?}
...
This spectral agreement extends beyond the $b\bar{b}$ channel: annihilation into $c\bar{c}$ provides comparably good fits (p-values of $\sim 0.4$--$0.5$), while leptonic channels ($\tau^+\tau^-$) are significantly disfavored ($p = 0.065$) but cannot be excluded at the 95\% confidence level~\cite{Calore:2014xka}.
...
In Section~\ref{sec:msp_paper}, we present a dedicated analysis that updates and extends this line of argument using 15.8 years of Fermi-LAT data and a comprehensive sample of 157 globular clusters, placing the most stringent constraints to date on pulsar interpretations of the GCE.
