...
While the theoretical model for $C_\ell$ is built from window functions $W_\gamma(z)$ that encode the redshift distribution of each source class (as developed in Section~\ref{sec:cross_correlations}), this information is observationally inaccessible from gamma-ray data alone: there is no way to decompose the measured $C_\ell$ into contributions from different redshifts.
Cross-correlating the gamma-ray map with an external gravitational tracer resolves both problems.
A galaxy catalog provides a redshift-dependent map of large-scale structure that correlates with the dark matter distribution at specific cosmic distances.
...
In contrast, blazars at energies of 50 GeV are distributed over $z \sim 0.1$--0.4, and at 1 TeV they are further suppressed at high redshift by extragalactic background light (EBL) absorption, but still do not overlap strongly with the very local galaxy distribution.
