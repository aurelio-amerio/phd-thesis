...
The angular power spectrum introduced in Section~\ref{sec:3.5} provides the mathematical language.
What remains is to demonstrate why cross-correlating the gamma-ray sky with a galaxy catalog offers a practical handle that the auto-correlation and spectral analyses alone cannot match.
The argument rests on three interlocking ideas: the redshift selectivity conferred by window function overlap (Section~\ref{sec:8.2.1}), the favorable noise properties that follow from combining two physically independent observables (Section~\ref{sec:8.2.2}), and the specific choice of low-redshift catalogs that maximizes both effects simultaneously (Section~\ref{sec:8.2.3}).
...
\begin{equation}
	C_\ell^{\gamma g} = \int \frac{d\chi}{\chi^2}\, W_\gamma(\chi)\, W_g(\chi)\, P_{\gamma g}\!
	\left(k = \frac{\ell}{\chi}, \chi\right),
\end{equation}
where $\chi(z)$ is the radial comoving distance, $W_\gamma$ and $W_g$ are the window functions of the gamma-ray and galaxy fields, respectively, and $P_{\gamma g}$ is the three-dimensional cross-power spectrum.
The key observation is that $C_\ell^{\gamma g}$ receives contributions only from the redshift range where both $W_\gamma$ and $W_g$ are simultaneously nonzero.
...
For unresolved astrophysical sources such as blazars, $W_\gamma$ depends on the luminosity function and the spectral energy distribution of the source class, modulated by extragalactic background light (EBL) absorption at high energies.
For dark matter annihilation, it is proportional to $\langle\sigma v\rangle\,\Delta^2(z)\,\left(\Omega_\mathrm{DM}\rho_c/m_\chi\right)^2\,(1+z)^3\,e^{-\tau(E,z)}$, where $\Delta^2(z)$ is the flux multiplier that captures the enhancement from DM clustering and substructure (see Section~\ref{sec:1.4})~\cite{Pinetti:2025hgd}.
...
Unresolved blazars --- high-synchrotron-peaked (HSP) and low-synchrotron-peaked (LISP) BL Lacs --- have their emission peak at $z \sim 0.3$--$0.5$ at $50\,\mathrm{GeV}$.
...
Figure~\ref{fig:window_main} illustrates this separation quantitatively for the benchmark scenario of~\cite{Pinetti:2025hgd}: the DM window functions are confined to low redshift, while blazar contributions extend to $z \sim 0.4$--$0.5$ at 50 GeV.
...
\end{equation}
where $f_\mathrm{sky}$ is the sky fraction observed, $C_N$ is the photon noise of the gamma-ray map, $C_{Ng}$ is the galaxy shot noise, and $B_\ell$ is the beam function of the gamma-ray instrument.
The noise terms $C_N$ and $C_{Ng}$ arise from the discrete, finite number of detected photons and catalogued galaxies, respectively.
These two noise sources are physically independent: photons emitted by gamma-ray sources carry no correlation with the positions of the external galaxy catalog.

This independence has a decisive consequence: the noise inflates the error bars on $C_\ell^{\gamma g}$ but does not add a systematic offset to the signal itself.
The auto-correlation gives a qualitatively different variance:
\begin{equation}
...
By contrast, $C_N$ in the cross-correlation variance multiplies the galaxy auto-spectrum $C_\ell^{gg} + C_{Ng}$, which is well measured independently; the DM signal is not hidden beneath the noise but simply requires accumulating sufficient signal-to-noise over multipoles and energy bins.
...
These are rare, bright point sources whose auto-correlation is a flat (Poissonian) $C_\ell$ from the 1-halo term, dwarfing any diffuse DM component.
...
For the DM signal, which is always diffuse and unresolved, the auto-correlation sensitivity therefore degrades as a function of the Poisson blazar floor.
...
The cross-correlation SNR for the astrophysical signal depends on the point-source sensitivity only weakly, through the window function of the resolved fraction, whereas the auto-correlation SNR is strongly sensitive to it~\cite{Pinetti:2025hgd}.
For DM, which is always diffuse, the cross-correlation sensitivity is essentially immune to the blazar detection threshold.

The power of this suppression is quantified directly in Paper~5~\cite{Pinetti:2025hgd}.
For 50\,h of CTAO extragalactic survey observation and the 2MASS catalog, the forecast cross-correlation upper bound on the DM annihilation cross section is approximately a factor of five better than the forecast auto-correlation bound for the $b\bar{b}$ channel.
Figure~\ref{fig:bounds1} shows the projected sensitivity curves for both approaches; the gap between the solid (cross-correlation) and dot-dashed (auto-correlation) lines quantifies this improvement.

The physical interpretation decomposes into the 1-halo and 2-halo structure of the three-dimensional cross-power spectrum, introduced in Section~\ref{sec:3.5} and detailed in~\cite{Camera:2012cj,Fornasa:2015qua}.
...
The Two Micron All-Sky Survey (2MASS) Extended Source Catalog contains approximately one million galaxies and has a redshift distribution peaking at $z \approx 0.072$, making it one of the shallowest but most complete all-sky galaxy surveys available~\cite{Fornasa:2015qua}. The 2MASS Redshift Survey (2MRS) is its spectroscopic subsample, extending to $z \sim 0.1$ with full spectroscopic redshifts, providing both a precise $dN_g/dz$ for modeling and higher galaxy density per solid angle in the local volume~\cite{Pinetti:2025hgd}.
...
Xia et al.
\ (2015) measured a cross-correlation signal at angular scales below one degree between the Fermi-LAT UGRB and four catalogs of large-scale structure.
...
A non-zero DM contribution was allowed by the data but degenerate with the mAGN and SFG components.
Setting upper limits by requiring that the DM term does not overproduce the observed signal, the analysis excluded the thermal annihilation cross section at 95\% confidence level for $m_\chi \lesssim 25\,\mathrm{GeV}$ ($b\bar{b}$ channel) under a conservative substructure model, and up to $\sim 600\,\mathrm{GeV}$ under an optimistic substructure assumption.
Lower bounds on the DM decay lifetime reached $\tau \gtrsim 10^{28}\,\mathrm{s}$ for a wide range of masses, roughly independent of particle mass.
These constraints are competitive with those from dSphs and galaxy clusters at the same epoch, and they arise from a different physical observable --- the spatial coherence of DM emission with the cosmic web --- making them a genuinely independent probe~\cite{Regis:2015zka}.

Beyond galaxy number counts, gravitational lensing maps offer an alternative tracer with independent systematics.
Cross-correlating the UGRB with weak-lensing convergence maps, Shirasaki et al.
\ (2014) found no significant signal and used the null result to set upper limits on $\langle\sigma v\rangle$, demonstrating that cosmic shear is useful for DM searches even in the absence of a detection~\cite{Shirasaki:2014noa}.
Camera et al.
\ (2013) showed theoretically that the shear window function overlaps well with the DM annihilation window function, making this combination particularly clean from an astrophysical foreground perspective~\cite{Camera:2012cj}.
The main practical challenge is the much larger noise of current weak-lensing surveys compared with galaxy number counts.

Looking ahead to CTAO, the choice of 2MASS and 2MRS as the reference catalogs is well motivated: their redshift distributions maximize the overlap with the DM window function in the energy range where CTAO is most sensitive ($\gtrsim 50\,\mathrm{GeV}$, where EBL absorption shrinks the gamma-ray horizon of astrophysical sources to $z \lesssim 0.2$)~\cite{Pinetti:2025hgd}.
