\subsection{The cross-correlation signal}
\label{sec:formalism}
...
We model the spectral energy distribution (SED) of these sources as power-laws with index $\varGamma$ and exponential cutoffs $E_\textrm{cut}$, whose specific parameter values are reported in Table~\ref{table:BLLACS}, together with the minimum and maximum luminosities of the population.
Since we focus on CTAO and thus on energies above approximately {20} GeV, 
other typical $\gamma$-ray emitters like misaligned AGN, star-forming galaxies, and flat-spectrum radio quasars can be neglected, since their contribution are dominant at lower energies \cite{DGRB-review}. Full details on the modeling of the astrophysical sources are provided in Appendix~\ref{app:model}, together with the expressions for their window functions and 3D cross-correlation power spectra, obtained within the halo model formalism.
...
as shown {in Figure \ref{fig:window_main} and in Figure \ref{fig:zWindows}} in Appendix~\ref{app:model}.
...
The cross-correlation APS signal has a variance $\smash{(\varDelta C^{\gamma g}_\ell)^2}$ given by~{\cite{Camera:2012cj,Camera_2015,Fornengo:2013rga}}
\begin{equation}
