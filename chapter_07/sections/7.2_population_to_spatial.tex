...
The first ingredient is a per-pixel test statistic quantifying how much the observed photon count in each pixel deviates from the background-only expectation.
Unlike the TS used by the \textit{Fermi}-LAT collaboration, which is computed locally in each region of interest and therefore carries a spatially varying significance, the TS adopted here is a single, globally coherent quantity defined identically for every pixel.
It is treated purely as a heuristic ``signal interest label'' whose probabilistic meaning is derived entirely from the comparison with simulated skies, not from an assumed analytic distribution.

The second ingredient is a two-sample Kolmogorov--Smirnov test that compares the cumulative TS distributions of the real sky and each synthetic realization above a minimum threshold $\mathrm{TS}^\star$.
At high $\mathrm{TS}^\star$, where bright sources dominate, the distributions agree well; as $\mathrm{TS}^\star$ is lowered, background systematics eventually cause the distributions to diverge, defining the depth to which the method can reliably probe.

The third ingredient is the Quality Factor (QF), defined as the fraction of the 5000 synthetic sky realizations that pass the KS test at a given $\mathrm{TS}^\star$ and significance level $\alpha$.
A high QF yields a conservative, high-purity selection of firing pixels; a lower QF extends the catalog deeper into the sub-threshold regime at the cost of including more spurious directions.
The pair $(\alpha, \mathrm{QF})$ thus controls the operating point of the probabilistic catalog, and the user can tune both meta-parameters to suit the downstream application.
