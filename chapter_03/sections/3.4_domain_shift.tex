...
We discuss both types in Section~\ref{sec:3.4.2}.
...
As a consequence, the spectral parameter distributions---flux, spectral index $\alpha$, and spectral curvature $\beta$---differ measurably between associated and unassociated sources \cite{Amerio:2025fhz}.
Whether this discrepancy arises from observational selection effects (a covariate shift), from genuinely different class compositions (a prior shift), or from a combination of both, cannot be determined a priori.
Any analysis that ignores this mismatch risks producing biased class prevalence estimates and, in the context of dark matter searches, unreliable upper bounds on the annihilation cross-section \cite{Amerio:2025fhz}.
...
Covariate shift corresponds to the first factorization $p(\mathbf{x}, k) = p(k|\mathbf{x})\,p(\mathbf{x})$.
...
In the Fermi-LAT context, the association procedure preferentially identifies bright sources with well-localized counterparts, so the associated sample is biased toward brighter sources relative to the full population \cite{2023RASTI...2..735M}.
The ratio $C(\mathbf{x}) = p_{\text{target}}(\mathbf{x})/p_{\text{train}}(\mathbf{x})$ between the target and training feature distributions characterizes the covariate shift \cite{Amerio:2025fhz}.
...
\end{equation}
where $b_d$ controls the steepness and $c_d$ the midpoint of the modulation along feature dimension $d$ \cite{Amerio:2025fhz}.
...
Prior shift---also called label shift---corresponds to the second factorization $p(\mathbf{x}, k) = p(\mathbf{x}|k)\,p(k)$.
...
Among Fermi-LAT sources, for instance, the fraction of Galactic sources is considerably larger in the unassociated sample than in the associated one, because a higher proportion of unassociated sources lie near the Galactic plane \cite{Amerio:2025fhz}.
...
In Paper~4, we construct the first such statistical model for Fermi-LAT sources \cite{Amerio:2025fhz}.
...
This approach draws on the classical dataset shift taxonomy of Moreno-Torres et al.~\cite{MorenoTorres2012AUV} and the quantification learning methods developed by Gonz\'alez et al.~\cite{10.1145/3117807_Gonzalez_quantification} and Moreo et al.~\cite{2024arXiv240100490M}, combining them into a unified framework that enables statistically rigorous upper bounds on the dark matter annihilation cross-section.
The full analysis is presented in Chapter~5.
