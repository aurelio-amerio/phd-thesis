...
Machine learning classifiers are typically trained on associated sources---whose class labels (e.g., pulsar or \gls{AGN_}) are known from their multi-wavelength counterparts---and then applied to unassociated sources to infer their nature (cf.\ Section~\ref{sec:3.3.1}).
...
The unassociated source distribution is modeled as a mixture of Galactic and extragalactic astrophysical components, $p(\mathbf{x}) = \sum_{k}\pi_k\,p(\mathbf{x}\,|\,k)$, whose class-conditional densities $p_{\text{assoc}}(\mathbf{x}|k)$ are estimated from the associated sources via \KDE, plus a possible dark matter subhalo component derived from Monte Carlo simulations.
