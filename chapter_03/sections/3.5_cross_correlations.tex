...
In an auto-correlation measurement, the photon noise $C_N$ adds directly to the signal, meaning that $C_\ell^{\gamma\gamma} + C_N$ must be measured and the noise accurately estimated and subtracted to recover the true clustering spectrum.
...
In Chapter~10, this formalism is applied to forecast the sensitivity of the Cherenkov Telescope Array Observatory (CTAO) to dark matter annihilation and decay, by cross-correlating the predicted gamma-ray emission with the 2MASS galaxy catalog.
As we will see, the cross-correlation channel outperforms the auto-correlation by approximately a factor of five in terms of sensitivity to the dark matter annihilation cross section, a direct consequence of the noise properties discussed above \cite{Pinetti:2025hgd}.
