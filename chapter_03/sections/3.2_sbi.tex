...
\blue{All three strategies provide \emph{amortized inference}: the computationally expensive simulation are performed once. 
Training a model does not require running the simulator for every new observation, and different model architechtures can be explored without additional calls to the simulator.
...
It is important to notice that the neural network also implicitly marginalizes over the vast latent space of the simulator, thus the trained surrogate is likely to preserve information that hand-crafted summary statistics might discard, making SBI extremely interesting for field-level inference. }
