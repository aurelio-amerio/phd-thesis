...
\blue{The more descriptive term \emph{implicit likelihood methods} is now preferred~\cite{Cranmer:2020}, and \emph{simulation-based inference} (SBI) is a term often used as an equally valid synonym. For convenience, and to align with the machine learning literature, we will refer to simulation-based inference throughout this thesis.}
...
This separation of the likelihood from the prior provides additional flexibility. \blue{The downside is that approximating a high-dimensional likelihood is generally more expensive than targeting the posterior directly, and posterior samples are not immediately available: they must be drawn from the trained likelihood surrogate through the additional MCMC step.}
...
\blue{All three strategies support \emph{amortized} variants: the computationally expensive simulations are performed once, and the trained model can then sample the posterior for any new observation without additional calls to the simulator~\cite{Cranmer:2020}.
...
\blue{We notice that validation would in principle also be required even when performing inference with a known likelihood, but it is sometimes overlooked in practice as it is often computationally intractable for standard MCMC methods.}
...
\blue{Comparing the empirical coverage with the nominal level across a range of credibility thresholds produces a coverage-vs-credibility plot; departures from the diagonal indicate over- or underdispersion of the posterior, and the discrepancy is formally quantified through the empirical calibration error~\cite{Cranmer:2020,Hermans:2021rqv}. When perfect calibration cannot be achieved, a slightly underconfident model (above the diagonal) is generally preferable in physics, since it yields conservative error bars.}
