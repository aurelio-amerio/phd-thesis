...
\blue{We refer to these regimes as \emph{background-dominated}, and the term is worth defining at the outset.}
\blue{The analyses of this thesis are rarely starved of photons: over more than fifteen years of observation, the Fermi-LAT has accumulated ample statistics across most of the sky.}
\blue{What limits the sensitivity is instead the astrophysical emission itself --- components far brighter than any plausible dark matter signal, whose morphology and normalization we can model only imperfectly.}
\blue{The challenge throughout is therefore systematic rather than statistical: the signal must be separated from backgrounds we cannot subtract exactly, and the methods of this chapter are built for that situation.}
...
\Gls{SBI_} bypasses the intractable likelihoods that arise in complex forward models, while neural networks learn directly from high-dimensional data when explicit physical models are unavailable.
...
Each method is presented at a conceptual level, with forward references to the chapters where the full technical details appear.
...
Section~\ref{sec:3.2} introduces the \SBI paradigm for problems where the likelihood cannot be evaluated analytically.
...
Section~\ref{sec:3.3} covers the machine learning tools---\glspl{CNN_} and density estimation---used for regression and classification tasks on gamma-ray data.
