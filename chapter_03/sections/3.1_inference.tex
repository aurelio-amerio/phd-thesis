...
Frequentist point estimation can remain tractable in the same regime through gradient-based optimization or iterative algorithms such as EM \blue{(Chapter~\ref{ch:5})}, which converge efficiently without exploring the full parameter space---but at the cost of forgoing the posterior structure that motivates the Bayesian approach in the first place.
