...
Frequentist point estimation can remain tractable in the same regime through gradient-based optimization or iterative algorithms such as EM \blue{(Chapter~\ref{ch:5})}, which converge efficiently without exploring the full parameter space---and what is given up is the posterior structure itself, not uncertainty quantification: confidence intervals remain available through the bootstrap (Section~\ref{sec:3.1.2}), at the cost of repeated refitting.
