...
\blue{The same asymptotic result can be used for the construction of confidence intervals, understood as the inverted test. Rather than fixing a single threshold and asking whether the null is rejected, we scan the parameter of interest and retain the range over which the profile-likelihood TS stays below the appropriate $\chi^2$ quantile.}
% \blue{For a 95\% confidence-level upper bound on a single parameter, the conventional choice is to identify the value at which $\mathrm{TS} = 3.84$---the 95\% quantile of $\chi^2_1$, which is conservative for a one-sided bound.}
\blue{Hypothesis testing and interval estimation are thus two uses of the same construction: the same test statistic and the same quantile quantify a detection significance and delimit a confidence interval.}
...
\blue{In Chapter~\ref{ch:5}, the uncertainty band on the dark matter annihilation cross-section bounds is obtained in exactly this way.}
...
\blue{Goodness-of-fit tests designed for this purpose come in parametric and nonparametric forms \cite{Cowan:1998ji}.}
\blue{Parametric tests assume a sampling distribution for a summary of the data; the classic example is Pearson's $\chi^2$ test, which compares binned counts with model expectations and refers the sum of squared normalized residuals to a $\chi^2$ distribution.}
\blue{Nonparametric tests instead compare data and model directly at the level of their distribution functions, without assuming a functional form for the discrepancy.}
\blue{Among the many nonparametric options, one which is employed in this thesis is the Kolmogorov--Smirnov (KS) test \cite{KS_test}.}
\blue{Its test statistic is the supremum of the absolute difference between the empirical cumulative distribution function of the data and the cumulative distribution predicted by the model.}
\blue{When the null model is fully specified, the null distribution of this statistic does not depend on the model being tested.}
