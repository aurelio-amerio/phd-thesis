...
\blue{The criterion derives from the \emph{Kullback--Leibler divergence}, $D_{\mathrm{KL}}(p \,\|\, q) = \int p(\mathbf{x}) \ln \left[ p(\mathbf{x}) / q(\mathbf{x}) \right] d\mathbf{x}$, which measures how strongly a distribution $q$ deviates from a reference distribution $p$.}
