...
The connection between parameters and data is established by the likelihood function $\mathcal{L}(\boldsymbol{\theta}) = p(\mathbf{x} \,|\, \boldsymbol{\theta})$, which quantifies how well a given parameter choice explains the observed data \cite{Cranmer:2020}.
...
In this form, with a background-only null hypothesis and an alternative that adds a source term, the \TS is the statistic used for \emph{source detection} in gamma-ray astronomy.
...
The criterion derives from the \emph{\gls{KL_} divergence}, $D_{\mathrm{KL}}(p \,\|\, q) = \int p(\mathbf{x}) \ln \left[ p(\mathbf{x}) / q(\mathbf{x}) \right] d\mathbf{x}$, which measures how strongly a distribution $q$ deviates from a reference distribution $p$.
...
\begin{equation}
	...
	\label{eq:bayes}
\end{equation}
where $p(\mathbf{x} \,|\, \boldsymbol{\theta}) = \mathcal{L}(\boldsymbol{\theta})$ is the same likelihood function used in the frequentist approach, and $p(\mathbf{x}) = \int p(\mathbf{x} \,|\, \boldsymbol{\theta}') \, p(\boldsymbol{\theta}') \, d\boldsymbol{\theta}'$ is the evidence.
