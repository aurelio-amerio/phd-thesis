...
This section introduces both frameworks from the shared foundation of the likelihood function, develops the key concepts \blue{used throughout this thesis}, and discusses when each paradigm is most appropriate (according to personal bias).
...
The following two subsections develop each approach, starting from a common notation and building toward the specific formulations \blue{employed in later chapters}.
...
\paragraph{\blue{Hypothesis testing and confidence intervals.}}

To compare competing hypotheses---for instance, whether a gamma-ray source exists at a given position---\blue{we construct} a likelihood-ratio test statistic (TS):
\begin{equation}
...
By Wilks' theorem, under \blue{mild} regularity conditions the TS is asymptotically distributed as a $\chi^2$ random variable with degrees of freedom equal to the difference in free parameters between the two hypotheses \cite{Wilks:1938}\blue{, which lets us convert a TS value into a statistical significance without resorting to explicit Monte Carlo simulations}.
\blue{Wilks' theorem is, strictly speaking, a result about hypothesis testing. In the context of this thesis, however, hypothesis testing mostly means setting confidence intervals or upper bounds on a parameter of interest. These are obtained by inverting the test: rather than fixing a single threshold and asking whether the null is rejected, we scan the parameter and retain the range over which the profile-likelihood TS stays below the appropriate $\chi^2$ quantile.}

\paragraph{\blue{Bootstrap confidence intervals.}}

\blue{The $\chi^2$ calibration of the TS rests on the regularity conditions of Wilks' theorem, and these can fail in situations that recur in gamma-ray analyses: when a parameter lies on a physical boundary---a flux or a cross-section that cannot be negative---or when the sample is small enough that the asymptotic regime has not yet set in.}
\blue{In such cases the bootstrap provides a general-purpose strategy \cite{Efron:1979}: we repeatedly refit the estimator, either on datasets resampled from the observed data (the \emph{nonparametric} bootstrap, or "with replacement") or on synthetic datasets drawn from the fitted model (the \emph{parametric} bootstrap), and read the confidence interval directly from the spread of the re-fitted estimates---the containment interval of their empirical distribution.}
\blue{In Chapter~\ref{ch:5}, the uncertainty band on the dark matter annihilation cross-section bounds is obtained using a parametric bootstrap.}

\paragraph{\blue{Goodness of fit.}}

\blue{A different question arises when we wish to assess the absolute compatibility of the data with a single model, without testing it against any specific alternative.}
\blue{Two general-purpose goodness-of-fit tests serve this purpose \cite{Cowan:1998ji}: Pearson's $\chi^2$ test and the Kolmogorov--Smirnov (KS) test \cite{KS_test}.}
\blue{Pearson's $\chi^2$ test compares binned counts with model expectations and refers the sum of squared normalized residuals to a $\chi^2$ distribution.}
\blue{The KS test instead compares data and model at the level of their cumulative distributions: its test statistic is the supremum of the absolute difference between the empirical cumulative distribution function of the data and the cumulative distribution predicted by the model.}
\blue{Because it operates on the unbinned data directly, the KS test is additionally independent of any binning choice.}
\blue{Moreover, when the null model is fully specified, the null distribution of its statistic does not depend on the model being tested.}
\blue{In practice, the model prediction is often available only as a finite simulated sample rather than an analytic cumulative distribution.}
\blue{The same statistic computed between the two empirical cumulative distribution functions is then known as the \emph{two-sample} form of the KS test.}
\blue{The KS test is therefore a convenient distribution-free diagnostic when no specific alternative is preferred.}

\paragraph{\blue{Non-nested model comparison.}}

\blue{The Wilks' likelihood-ratio test presumes that the two hypotheses are nested, with one model recovered as a special case of the other; when this is not so, we need a criterion able to rank models of different structure.}
\blue{Information criteria fill this gap.}
...
Model selection then proceeds by minimising the AIC across the candidates; \blue{the differences in AIC quantify how strongly each model is disfavoured relative to the best one.}
...
\begin{itemize}
	...
	\item In Chapter~\ref{ch:5}, a profile likelihood test statistic is used with Wilks' theorem to set 95\% CL upper bounds on the dark matter annihilation cross-section\blue{, with the uncertainty band on those bounds derived from a parametric bootstrap}; the AIC is additionally used to compare competing models for the unassociated source population.
...
\begin{equation}
	...
	\label{eq:bayes}
\end{equation}
where $p(\mathbf{x} \,|\, \boldsymbol{\theta}) = \mathcal{L}(\boldsymbol{\theta})$ is the same likelihood function used in the frequentist approach, and $p(\mathbf{x}) = \int p(\mathbf{x} \,|\, \boldsymbol{\theta}') \, p(\boldsymbol{\theta}') \, d\boldsymbol{\theta}'$ is the evidence, a normalization constant that integrates the likelihood weighted by the prior over the full parameter space \cite{bishop}.
...
\subsection{\blue{Frequentist versus Bayesian}}
\label{sec:3.1.4}
...
\blue{Neither is inherently superior to the other.}
\blue{Both provide uncertainty quantification---confidence intervals constructed via the profile likelihood or the bootstrap on one side, posterior credible intervals on the other.}
\blue{Both can incorporate external constraints.}
\blue{In the Bayesian framework these enter through the prior---restricting fluxes to non-negative values, for example, or spectral indices to physically motivated ranges.}
\blue{A frequentist analysis imposes the same requirements through bounded parameter ranges or constrained fits.}
\blue{Bounding a parameter is not entirely free of subtleties, however: when the true value may lie close to the boundary, standard interval constructions can undercover or return empty intervals.}
\blue{The unified approach of Feldman and Cousins \cite{Feldman:1997qc} resolves this by transitioning smoothly between upper limits and two-sided intervals while guaranteeing correct coverage.}
\blue{The choice between them is therefore pragmatic rather than fundamental, driven by the structure of the problem, computational considerations, and---above all---the conventions of the relevant research community.}
...
\blue{Cosmology and large-scale-structure analyses, by contrast,} have largely adopted Bayesian Markov Chain Monte Carlo (MCMC) methods \blue{\cite{Trotta:2008qt}}.
...
\blue{In this thesis, we follow the convention of the community relevant to each analysis.}

In the limit of abundant, constraining data, \blue{the distinction fades altogether}: the likelihood dominates any reasonable prior, and Bayesian credible intervals coincide numerically with frequentist confidence intervals.
...
\blue{These are validated against} a frequentist analysis of the same data\blue{, in which} confidence intervals are extracted from the empirical distribution of residuals across the validation set.
