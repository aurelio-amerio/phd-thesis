...
The connection between parameters and data \blue{is established by} the likelihood function $\mathcal{L}(\boldsymbol{\theta}) = p(\mathbf{x} \,|\, \boldsymbol{\theta})$, which quantifies how well a given parameter choice explains the observed data \cite{Cranmer:2020}.
...
The most common frequentist estimator is the \gls{MLE_}, which identifies the parameter values that maximize the likelihood:
\begin{equation}
...
To compare competing hypotheses---for instance, whether a gamma-ray source exists at a given position---we construct a likelihood-ratio \gls{TS_}:
\begin{equation}
...
Two general-purpose goodness-of-fit tests serve this purpose \cite{Cowan:1998ji}: Pearson's $\chi^2$ test and the \gls{KS_} test \cite{KS_test}.
...
The \gls{AIC_} \cite{Akaike:1974} is defined as
\begin{equation}
...
\blue{The criterion derives from the \emph{\gls{KL_} divergence}, $D_{\mathrm{KL}}(p \,\|\, q) = \int p(\mathbf{x}) \ln \left[ p(\mathbf{x}) / q(\mathbf{x}) \right] d\mathbf{x}$, which measures how strongly a distribution $q$ deviates from a reference distribution $p$.}
The \AIC provides an estimator of the relative \KL divergence between each candidate model and the data-generating process, with the $2k$ term penalising additional parameters and thus discouraging overfitting.
...
\begin{itemize}
	\item In Chapter~\ref{ch:4}, the \gls{MSP_} luminosity function is constrained via \MLE applied to an integrated likelihood, and model comparison is performed through $-2\Delta\ln\mathcal{L}$ values.
	\item In Chapter~\ref{ch:5}, a profile likelihood \TS is used with Wilks' theorem to set 95\% confidence-level upper bounds on the dark matter annihilation cross-section, with the uncertainty band on those bounds derived from a parametric bootstrap; the \AIC is additionally used to compare competing models for the unassociated source population.
	...
	\item In Chapter~\ref{ch:8}, a $\Delta\chi^2$ test statistic compares astrophysical-only and astrophysical-plus-DM hypotheses for the cross-correlation \gls{APS_}.
\end{itemize}
...
In Chapter~\ref{ch:6}, the Bayesian framework is implemented via \SBI to reconstruct the source-count distribution $dN/dS$ from gamma-ray sky maps, with posterior uncertainty estimates validated against an independent frequentist cross-check (Section~\ref{sec:3.1.4}).
...
Cosmology and large-scale-structure analyses, by contrast, have largely adopted Bayesian \gls{MCMC_} methods \cite{Trotta:2008qt}.
...
In Chapter~\ref{ch:6}, a \SBI approach provides Bayesian posterior estimates on the reconstructed $dN/dS$.
