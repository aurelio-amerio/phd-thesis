...
Readers familiar with statistical inference may proceed directly to the application previews, which connect the general formalism to its specific use in Papers~1--5.
...
\end{equation}
with $k_a$ the observed counts and $\lambda_a(\boldsymbol{\theta})$ the model prediction in pixel $a$ \cite{Amerio:2023dns}.
...
These properties make the frequentist approach the natural default for standard Fermi-LAT analyses, where the test statistic is used to determine source detection thresholds and to set upper bounds on new physics contributions \cite{Amerio:2025sub, Amerio:2024msp}.
