...
**Response:** ✔✔ Double-checked (NotebookLM against the thesis source notebook + an
independent non-Hooper literature sweep, per author request). The reconciled picture:

1. *GC replenishment is empirically solid, not just plausible.* MSP/LMXB abundance
   correlates tightly with the stellar encounter rate $\Gamma \propto \int \rho^2/\sigma$:
   Pooley et al. 2003 (astro-ph/0305003), Bahramian et al. 2013 (arXiv:1302.2549),
   de Menezes et al. 2019 ($N_{\rm MSP} \propto \Gamma^{0.64}$), and in gamma rays Hui
   et al. 2011 / Feng et al. 2024 ($L_\gamma \propto \Gamma^{0.70}$). CMC cluster models
   (Ye et al. 2019, arXiv:1902.05963 — Northwestern group, non-Hooper) show NS binary
   formation continuing over a Hubble time. Our own paper's preferred
   $\langle N_{\rm MSP}\rangle \propto \Gamma_e^{0.7}$ scaling is this same physics.
2. *Bulge field: no channel.* Per-unit-mass encounter rate is down by a factor
   $10^4$–$10^5$ (ρ ~ 10–100 vs 10⁵–10⁶ M⊙/pc³, σ ~ 100–150 vs ~10 km/s); empirically,
   GC specific gamma-ray emissivity exceeds that of bulge stars by 10–1000×.
3. *NSC: Eckner is partly right.* Dynamical formation demonstrably operates in the
   central parsec (Muno et al. 2005; Hailey et al. 2018; Generozov et al. 2018,
   arXiv:1804.01543 — high σ suppresses exchange/primordial channels but tidal capture
   survives). **However, Eckner's own paper (Eckner et al. 2018, arXiv:1711.05127,
   Sec. 8.2) quantifies the combined bulge+NSC dynamical contribution at the percent
   level of the GCE, confined to the innermost ~degree** — it cannot touch the extended
   10° signal. Citing his own bound back to him is the cleanest possible reply.
4. *⚠ Hooper-bias flag, as the author suspected — but on the fading side.* "The
   deposited population would have faded" is directionally right but its *magnitude* is
   contested: Fragione, Antonini & Gnedin 2018 (arXiv:1709.03534 = `Fragione:2017rsp`)
   include spin-down with the *observed* τ distribution (long ≳5 Gyr tail) and find the
   deposited population still bright enough to reproduce the GCE — and explicitly state
   that Hooper & Linden's τ ≈ 0.25 Gyr distribution "underestimates the final MSP
   gamma-ray luminosity by roughly one order of magnitude" and is in tension with
   observations. Ye & Fragione 2022 (`Ye:2022yxt`) agree. So the thesis must present
   fading as real physics with a contested amplitude, not as an established escape
   route or an established irrelevance. The existing hedge at line 73 ("How much fainter
   … remains an open question") is exactly right — keep it and give it the Fragione
   citation.
...
> channel. The disk forms new binaries through ongoing star formation, while globular
> clusters form them dynamically in their dense cores --- a channel that remains active
> today, as evidenced by the tight correlation between a cluster's MSP content and its
> stellar encounter rate~\cite{<Pooley2003>,<Bahramian2013>}. The diffuse bulge field
> offers neither channel: its encounter rate per unit stellar mass falls short of
> cluster cores by four to five orders of magnitude. Dynamical formation does operate
> in the nuclear star cluster, whose central densities rival those of cluster cores,
> but its contribution to the GCE is bounded at the percent level and confined to the
> innermost degree~\cite{<Eckner2018>}. A population deposited in the bulge by
> disrupted clusters, having lost access to any replenishment channel, would therefore
> only have spun down and faded since its deposition.}"
>
> And at line 73, strengthen the existing hedge:
> "How much fainter the inner-Galaxy pulsars could realistically be, however, remains
> an open question\blue{: models adopting the spin-down timescale distribution observed
> in globular clusters find that the deposited population, though faded, can remain
> bright enough to account for a substantial fraction of the
> excess~\cite{Fragione:2017rsp,Ye:2022yxt}}."
>
> Bib to fetch from InspireHEP: astro-ph/0305003 (Pooley et al.), arXiv:1302.2549
> (Bahramian et al.), arXiv:1711.05127 (Eckner et al. 2018 — check if already present;
> likely cited elsewhere in Ch. 4). `Fragione:2017rsp` and `Ye:2022yxt` already exist.
> Keep `\cite{Fragione:2017rsp,Ploeg:2020jeh,Ye:2022yxt}` on the evolutionary-models
> sentence at line 72.
...
- Supporting detail from the E-4.4 verification: the nuclear star cluster itself is
  largely assembled from these same disrupted clusters (Gnedin, Ostriker & Tremaine
  2014, arXiv:1308.0021 — check bib key `Gnedin:2013cda`), which makes the
  deposition picture self-consistent and preempts the "why not NSC MSPs" objection.
