...
These leptons upscatter ambient low-energy photons to gamma-ray energies via \textit{inverse Compton scattering} (ICS; see Section~\ref{sec:leptonic}) \cite{Blumenthal:1970gc}.
...
Despite this additional layer of modelling uncertainty, secondary emission plays a central role in the interpretation of the Galactic Center excess discussed in Chapter~\ref{ch:4}, where the identification of the signal relies on subtracting the diffuse gamma-ray background dominated by cosmic-ray ICS and bremsstrahlung in the dense inner Galaxy.
...
For axions and axion-like particles (ALPs), introduced in Section~\ref{sec:1.2.1}, the observable of choice is not an annihilation flux but the \emph{spectral irregularities} that photon--ALP mixing in astrophysical magnetic fields imprints on bright gamma-ray sources~\cite{Fermi-LAT:2016nkz}.
The preferred targets shift accordingly, toward active galactic nuclei embedded in strongly magnetized galaxy clusters and transient events such as supernovae~\cite{Payez:2014xsa} (cf.\ Section~\ref{sec:extragalactic_sources}).
For primordial black holes, the signature is instead \emph{Hawking evaporation}, which makes the diffuse MeV background~\cite{Ballesteros:2019exr}, the individual nearby objects entering their terminal evaporation phase~\cite{Fermi-LAT:2018pfs}, and the short GeV--TeV bursts marking its final instants~\cite{HAWC:2019wla} some of the most relevant channels.
...
\textbf{Charged cosmic rays} --- electrons, positrons, antiprotons, and heavier anti-nuclei --- suffer from a fundamental limitation: galactic magnetic fields randomize their arrival directions\blue{.
...
These parameters overlap with those invoked for the Galactic Center excess (Chapter~\ref{ch:4}), but the significance drops below $\sim 2\sigma$ once systematic uncertainties in the propagation model and production cross-sections are included.
...
The original combined analysis of 15 dSphs with six years of data excluded the thermal relic cross-section for dark matter masses in the range $\sim 10$--$100$ GeV in the $b\bar{b}$ channel \blue{\cite{Fermi-LAT:2015att,2024PhRvD.109f3024M}}; below $\sim 10$ GeV the Fermi-LAT effective area drops steeply and the constraints weaken.
...
The Galactic Center excess, first reported in 2009, remains the most debated (its spectrum and morphology are described in Section~\ref{sec:targets}) \cite{Daylan:2014rsa,Cirelli:2024ssz}.
...
Chapter~\ref{ch:4} of this thesis contributes directly to this debate by challenging the leading astrophysical interpretation of the excess as an unresolved population of millisecond pulsars.
...
Fermi-LAT probes the GeV window where the Galactic Center excess resides, but the TeV regime remains only marginally explored.
...
Closing this window is a primary science goal of the Cherenkov Telescope Array Observatory (CTAO), whose Galactic Center observations are forecast to reach the thermal cross-section at these masses~\cite{CTA:2020qlo}.
