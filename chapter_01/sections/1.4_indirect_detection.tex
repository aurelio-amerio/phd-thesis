...
From the particle physics side, self-interacting dark matter \blue{(introduced in Section~\ref{sec:1.2.1})}, with $\sigma/m \approx 0.5$--$10$ cm$^2$/g, transfers heat to halo centers and produces cores of the observed size \cite{Spergel:1999mh,Tulin:2017ara,Bullock:2017xww}, while ultralight ``fuzzy'' dark matter generates solitonic cores through quantum pressure on scales comparable to the de Broglie wavelength \cite{Hu:2000ke,Cirelli:2024ssz}.
...
The inner few degrees of the Galaxy feature a bright diffuse emission from cosmic-ray protons and electrons scattering off interstellar gas (through pion production and bremsstrahlung) and ambient radiation fields (through inverse Compton scattering), together with numerous point sources --- supernova remnants, pulsars, and the central supermassive black hole \cite{Hooper:2024,Cirelli:2024ssz}.
Models of this diffuse foreground (see Section~\ref{sec:gde_model}) are constructed from gas maps and the locally measured cosmic-ray spectrum, and while they capture the broad features of the observed emission, they cannot account for its detailed spectral and morphological structure \cite{Hooper:2024}.
...
\blue{Dwarf spheroidals are among the most dark matter--dominated objects known (Section~\ref{sec:1.1.1}): their shallow gravitational potential wells allowed early supernova feedback to expel most of the baryonic gas, halting star formation and leaving behind systems with extreme mass-to-light ratios \cite{Cirelli:2024ssz}.}
...
The advent of digital surveys --- SDSS, DES, and Gaia --- has brought the total to about fifty confirmed ultrafaint dwarfs, with roughly ten more candidates awaiting spectroscopic confirmation \cite{Cirelli:2024ssz,Pace:2024sys}.
...
\blue{These stacked analyses exclude the thermal relic cross-section for $b\bar{b}$ masses below $\sim 100$ GeV; the current numbers, and their evolution from 15 to 50 dwarfs, are collected in Section~\ref{sec:status} \cite{Fermi-LAT:2015att,2024PhRvD.109f3024M}.}
...
We argued that dark matter annihilation and decay could produce stable Standard Model particles, not only gamma-rays.
Each messenger opens a complementary window onto the dark sector, with its own advantages and limitations.
...
An independent and increasingly powerful constraint comes from the \CMB: dark matter annihilation during and after recombination injects energy into the photon--baryon plasma, distorting the CMB temperature and polarisation anisotropies in a way that is tightly bounded by Planck data \cite{Ade:2015xua,Cirelli:2024ssz}.
...
The current status is best summarized as a set of progressively tightening bounds on the annihilation cross-section and decay lifetime, with a few anomalies whose dark matter interpretations have been challenged by plausible astrophysical alternatives.
...
\blue{The Galactic Center excess, first reported in 2009, remains the most debated of these anomalies (its spectrum and morphology are described in Section~\ref{sec:targets}) \cite{Daylan:2014rsa,Cirelli:2024ssz,Hooper:2024}.}
