...
\begin{figure}[t]
	...
	\caption{Comoving abundance $Y \equiv n_\chi/s$ as a function of the dimensionless inverse temperature $x \equiv M_\chi/T$.
		...
		The solid curves show the actual abundance for three values of the velocity-averaged annihilation cross section, illustrating that stronger annihilation cross sections track equilibrium longer and freezes out at a smaller residual abundance.
...
The inverse relation of Eq.~\eqref{eq:relic_abundance} is illustrated in Figure~\ref{fig:thermal_relic}: the orange band marks the cross section that reproduces the measured abundance, while smaller (larger) values freeze out too early (too late) and overproduce (underproduce) dark matter.
...
The upper end of the mass window has a more universal origin, which is often refferred to as the \emph{unitarity bound}.
...
The bounds derived in Section~\ref{sec:1.2.3} assumed the simplest possible freeze-out scenario: a single stable species, two-body $s$-wave annihilation with velocity-independent cross section, and weak-scale couplings.
...
The first is \emph{co-annihilation}: if the dark sector contains additional particles whose masses satisfy $M' - M_\chi \lesssim$ a few$\,\times T_\mathrm{fo}$, where $T_\mathrm{fo}$ is the freeze-out temperature, the heavier partners remain Boltzmann-abundant during freeze-out and contribute to depleting the dark-matter density through both their own annihilations and $\chi$-partner co-annihilation processes~\cite{Ellis:1983ew,Griest:1990kh}.
