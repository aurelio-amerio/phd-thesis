...
The thermal-relic cross section, established by the freeze-out calculation, \blue{is} the primary benchmark for the indirect-detection analyses throughout this thesis.
...
\blue{We survey the main candidate classes in order of increasing mass, reading Figure~\ref{fig:dm_mass_landscape} from left to right, closing with the question of whether dark matter is truly cold and collisionless.}
...
\blue{At the light end of the spectrum sit \emph{axions}, which have drawn growing attention from the community as direct-detection limits on \glspl{WIMP_} have tightened.
...
\blue{Not every candidate was ever in thermal contact with the primordial plasma (like in the freeze-out scenario).
...
\blue{Several orders of magnitude heavier, in the GeV--TeV range, sit the \glspl{WIMP_} that are the primary focus of this thesis and one of the most popular cold dark matter candidates; they interact with Standard Model particles through weak-scale couplings.}
\blue{Unlike \glspl{FIMP_}, they reach full thermal equilibrium with the primordial plasma and are} produced through thermal freeze-out in the early Universe, a mechanism we describe in detail in Section~\ref{sec:1.2.2}.
...
\blue{A question independent of the specific candidate is whether dark matter departs at all from the cold, collisionless scenario on which $\Lambda$CDM is built.}
...
For the GeV-scale candidates relevant to the Galactic Center excess analyzed in Chapter~\ref{ch:4}, by contrast, the standard thermal cross section fits the observed signal without invoking Sommerfeld enhancement, as Daylan et al.~\cite{Daylan:2014rsa} showed explicitly for a $\sim \blue{35}$--$\blue{50}\,\mathrm{GeV}$ \WIMP annihilating at $\langle\sigma v\rangle \approx (1$--$3)\times 10^{-26}\,\mathrm{cm}^3/\mathrm{s}$.

\paragraph{\blue{Bound-state formation.}}
\blue{The same light mediator that produces the Sommerfeld enhancement supports a second non-perturbative effect: two dark matter particles can emit a mediator and fall into a bound state, in close analogy with hydrogen recombination~\cite{vonHarling:2014kha}.
