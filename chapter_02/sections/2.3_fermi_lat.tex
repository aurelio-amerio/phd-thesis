...
Launched in 2008, the \textit{Fermi}-LAT has provided the most sensitive all-sky survey in the GeV domain to date, operating continuously for over fifteen years.
...
In this section, we review the fundamental design of the \textit{Fermi}-LAT, the energy-dependent instrumental response functions that govern its spatial and energy resolution, and the resulting catalogs of point sources that serve as the foundation for both individual target searches and statistical population studies.


\subsection{Instrument Overview}
\label{sec:fermi_lat}

The \textit{Fermi}-LAT is a pair-conversion telescope designed to measure cosmic gamma rays.
...
\begin{figure}[t]
	...
	\caption{Schematic diagram of the \textit{Fermi}-LAT, showing the tracker modules, calorimeter, and the surrounding Anticoincidence Detector.
...
The entire tracker array is covered by the \gls{ACD_}, a segmented shield of 89 plastic scintillator tiles read out by wavelength-shifting fibers and photomultiplier tubes.
The \ACD provides a charged-particle detection efficiency exceeding 0.9997 averaged over its area, enabling rejection of the charged cosmic-ray background that outnumbers gamma rays by several orders of magnitude \cite{Atwood:2009ez}.
...
This hardware configuration enables the \textit{Fermi}-LAT to achieve substantial improvements over previous-generation instruments.
The \textit{Fermi}-LAT operates across an energy range spanning from 20~MeV to over 300~GeV.
...
The physical characteristics of the \textit{Fermi}-LAT---its tracking efficiency, multiple scattering within the conversion foils, and incomplete shower containment---are encapsulated in the \glspl{IRF_}.
The \glspl{IRF_} map the true energy, direction, and conversion location of incident gamma rays to their reconstructed quantities.
Three \IRF components are particularly relevant for the analyses in this thesis: the \gls{PSF_}, the \gls{EDISP_}, and the effective area.

\PSF describes the angular resolution of the telescope, quantifying the probability distribution of reconstructed directions around the true source position.
...
Consequently, the \PSF is strongly energy-dependent, as shown in Figure~\ref{fig:lat_psf}: at 100~MeV, the 68\% containment angle spans approximately $3.5^\circ$ to $5^\circ$; it tightens to roughly $0.6^\circ$ at 1~GeV and reaches an asymptotic limit of less than $0.15^\circ$ above 10~GeV \cite{Atwood:2009ez}.
Off-axis, the containment angle degrades by up to a factor of 1.7 at $55^\circ$ incidence.
The broad \PSF at lower energies creates a severe source confusion limit: because the spatial profiles of nearby point sources overlap with each other and with the intense \GDE, it becomes statistically difficult to disentangle individual faint objects.
...
\begin{figure}[t]
	\centering
	\includegraphics[height=0.49\textwidth, angle=90]{chapter_02/figures/lat_psf.pdf}
	\includegraphics[width=0.49\textwidth]{chapter_02/figures/gPsfAve68Energy_P8R3_SOURCE_V3psf.png}
	\caption{\textit{Left}: The 68\% containment angle of the \PSF as a function of energy at normal incidence (solid) and at a $60^\circ$ off-axis incidence angle (dashed), for photons converting in the thin \textit{front} section of the tracker.
	Photons converting in the thick \textit{back} section have a \PSF approximately twice as wide at 1~GeV.
	\textit{Right}: the 68\% containment angle of the \PSF as a function of energy for the four \PSF event types (PSF0--PSF3, from broadest to narrowest).
	Credit: Left from \cite{Atwood:2009ez} and right from~\cite{Ajello_2021}.
	}
	\label{fig:lat_psf}
\end{figure}

\EDISP quantifies the accuracy of the reconstructed photon energy.
...
For most analyses in this thesis, the energy resolution is sufficiently precise that \EDISP is not a dominant systematic.

Not every gamma ray that enters the \textit{Fermi}-LAT produces a usable event: some pass through without converting, others are rejected during background filtering.
...
At normal incidence, it rises steeply from $\sim$100~cm$^2$ at 20~MeV to a broad plateau of $\sim$8,000--9,500~cm$^2$ between 1~GeV and several hundred GeV, before declining at the highest energies \cite{Atwood:2009ez}. See Figure~\ref{fig:effective_area} for the energy and incidence angle dependence of the effective area.
For comparison, the \textit{Fermi}-LAT's overall footprint is $1.8 \times 1.8~\mathrm{m}^2$; the peak effective area of $\sim$9,500~cm$^2$---roughly a third of the geometric area---reflects the compounded probabilities of pair conversion in the tungsten, successful track reconstruction, and survival of background-rejection cuts.
For a survey instrument, however, the relevant figure of merit is not the on-axis effective area alone, but the \emph{acceptance}---the effective area integrated over all incident directions.
While the effective area quantifies the detection efficiency for photons arriving from a single direction, the acceptance captures the instrument's total photon-collecting power across the full field of view simultaneously.
The \textit{Fermi}-LAT's acceptance peaks at $\sim$2.5~m$^2$~sr above 1~GeV \cite{Atwood:2009ez}, a direct consequence of the large 2.4~sr field of view enabled by the tracker's compact aspect ratio.
Combined with the scanning survey mode described in Section~\ref{sec:fermi_lat}, this large acceptance allows the \textit{Fermi}-LAT to achieve nearly uniform full-sky exposure every three hours, operating as a continuous all-sky monitor rather than a pointed observatory.


%% figure here 
\begin{figure}[t]
	\centering
	\includegraphics[width=0.49\textwidth]{chapter_02/figures/gAeffEnergy_P8R3_SOURCE_V3fb.png}
	\includegraphics[width=0.49\textwidth]{chapter_02/figures/gAeffTheta_10GeV_P8R3_SOURCE_V3fb.png}
	\caption{\textit{Left}: Effective area as a function of the energy of the incident photon. \textit{Right}: Effective area at 10 GeV as a function of the angle of incidence.
	Credit: Fermi-LAT collaboration~\cite{Ajello_2021}.
	}
	\label{fig:effective_area}
\end{figure}


The accuracy of the \glspl{IRF_} relies directly on the underlying event reconstruction algorithms.
The Pass~8 analysis framework is a complete reworking of the \textit{Fermi}-LAT event-level reconstruction \cite{Fermi-LAT:2013jgq}.
...
With the \glspl{IRF_} established, we now turn to how the \textit{Fermi}-LAT data are translated into astrophysical catalogs of gamma-ray sources.
Converting raw photon events into measurements of individual sources requires disentangling finite objects from the structured diffuse backgrounds.
...
The observed sky is modeled through the template decomposition introduced in Section~\ref{sec:gde_model}---the structured \GDE plus a spatially uniform isotropic component---augmented with individual point sources, each parametrized by a position and a spectral model.
...
To evaluate the significance of a candidate point source, the framework computes the \gls{TS_}, defined as \[ TS = 2\ln\frac{\mathcal{L}_1}{\mathcal{L}_0}, \] where $\mathcal{L}_1$ is the maximum likelihood of the model with the candidate source included and $\mathcal{L}_0$ is the maximum likelihood of the null hypothesis without it \cite{Mattox:1996zz}.
...
The most recent published incremental release, 4FGL-DR4, extends the dataset to 14 years and contains 7,195 sources \cite{Ballet:2023qzs}, with more recent data releases continuing to grow the catalog.
Throughout this evolution, the detection threshold has remained at $TS > 25$, so the growing source count is driven primarily by longer exposure, improvements in the galactic foreground modeling, and improved sensitivity from Pass~8 event reconstruction.
...
These spectral characterizations---in particular the flux, spectral index, and curvature from the log-parabola fits---form the feature space used in Chapter~\ref{ch:5} for a population-level study of unassociated sources.
...
In the 4FGL-DR4, 4022 sources are identified or associated with extragalactic objects and 610 with Galactic source classes.
However, a substantial fraction---2563 sources---lacks any plausible multi-wavelength counterpart and remains unassociated \cite{Ballet:2023qzs}.
...
This population likely consists of faint blazars and pulsars below the sensitivity of current multi-wavelength surveys, but it is also the primary search pool for exotic gamma-ray emitters such as dark matter subhalos.
As mentioned in Chapter~\ref{ch:1} and detailed further in Chapter~\ref{ch:5}, $\Lambda$CDM cosmology predicts a large population of low-mass Galactic subhalos that are too low in mass to retain gas or form stars, and are therefore invisible at all wavelengths except gamma rays, where \WIMP annihilation could produce a faint, spectrally soft signal closely resembling an unassociated \textit{Fermi}-LAT point source. Separating such candidates from the astrophysical sources within the 4FGL catalog is the central challenge addressed in Chapter~\ref{ch:5}.
