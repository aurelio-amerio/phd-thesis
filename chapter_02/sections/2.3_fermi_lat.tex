...
\begin{figure}[t]
	...
	\caption{\textit{Left}: The 68\% containment angle of the Point Spread Function as a function of energy at normal incidence (solid) and at a $60^\circ$ off-axis incidence angle (dashed), for photons converting in the thin \textit{front} section of the tracker.
...
The LAT's acceptance peaks at $\sim$2.5~m$^2$~sr above 1~GeV \cite{Atwood:2009ez}, a direct consequence of the large 2.4~sr field of view enabled by the tracker's compact aspect ratio.
Combined with the scanning survey mode described in Section~\ref{sec:fermi_lat}, this large acceptance allows the LAT to achieve nearly uniform full-sky exposure every three hours, operating as a continuous all-sky monitor rather than a pointed observatory.
...
\blue{The Pass~8 analysis framework is a complete reworking of the LAT event-level reconstruction} \cite{Fermi-LAT:2013jgq}.
...
With the IRFs established, we now turn to how the LAT data are translated into astrophysical catalogs of gamma-ray sources.
Converting raw photon events into measurements of individual sources requires disentangling \blue{finite} objects from the structured diffuse backgrounds.
...
\blue{The observed sky is modeled through the template decomposition introduced in Section~\ref{sec:gde_model}---the structured Galactic diffuse emission plus a spatially uniform isotropic component---augmented with individual point sources, each parametrized by a position and a spectral model.}
...
The most recent published incremental release, 4FGL-DR4, extends the dataset to 14 years and contains \blue{7,195} sources \cite{Ballet:2023qzs}, with more recent data releases continuing to grow the catalog.
...
In the 4FGL-DR4, \blue{4022} sources are identified or associated with extragalactic objects and \blue{610} with Galactic source classes.
However, a substantial fraction---\blue{2563} sources---lacks any plausible multi-wavelength counterpart and remains unassociated \cite{Ballet:2023qzs}.
