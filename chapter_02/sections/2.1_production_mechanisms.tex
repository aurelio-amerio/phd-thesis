...
% Before cataloging individual point sources or attempting to extract information from the faint extragalactic emission, one must understand the physical processes that generate the intense Galactic foreground introduced above.
...
Both families of processes operate within the \gls{ISM_}, the mix of gas, dust, radiation, magnetic fields, and cosmic rays that fills the space between stars.
...
This gas supplies the collision targets for the hadronic interactions and bremsstrahlung discussed below, while the \emph{interstellar radiation field}---starlight, infrared emission, and the \gls{CMB_}---provides the seed photons for \gls{ICS_}.

\subsection{Hadronic Emission}
\label{sec:hadronic}

High-energy cosmic-ray protons and heavier nuclei interact with the \ISM through the strong nuclear force, primarily colliding with ambient hydrogen gas.
...
Once this threshold is crossed, the total inelastic cross section increases only mildly with energy, rising from $\sigma_\text{inel}^{pp} \approx 30$--$40$ mb at $E_p \lesssim 10$ GeV to $\sigma_\text{inel}^{pp} \approx 100$ mb near $E_p \sim 1$ EeV.
...
\begin{figure}[t]
	...
	\caption{Measurements of the total and inelastic proton-proton cross section, where the so-called pion bump is clearly visible. Adapted from \cite{ParticleDataGroup:2022pth}.}
...
This universal spectral feature, known as the \emph{pion bump}, is the characteristic spectral signature of hadronic cosmic-ray interactions with gas and has been observed in both \glspl{SNR_} and the Galactic Plane emission \cite{Fermi-LAT:2013iui}, see Figure~\ref{fig:pp_cross_section}.
...
Hadronic emission dominates the \gls{GDE_} along the Galactic Plane, where the column density of interstellar gas is highest.
As a result, it constitutes the primary astrophysical background for any dark matter search targeting the inner Galaxy, with consequences we quantify in Section~\ref{sec:gde_model}.
While hadronic emission dominates wherever gas is abundant, leptonic processes---\ICS, bremsstrahlung, and synchrotron radiation---provide the complementary diffuse components, particularly at high Galactic latitudes.

\subsection{Leptonic Emission}
\label{sec:leptonic}

Relativistic electrons and positrons produce high-energy photons through three primary mechanisms: \ICS, bremsstrahlung, and synchrotron radiation.
...
Relativistic electrons can up-scatter low-energy ambient photons---the \CMB, infrared dust emission, and starlight---into the X-ray and gamma-ray bands \cite{Blumenthal:1970gc}.
\blue{The behavior of this process depends critically on the energy $\epsilon$ of the target photon, through the dimensionless ratio $\epsilon E_e / m_e^2$. Up to a factor of order unity, this ratio is the photon energy in the electron rest frame, measured in units of the electron mass.}
...
\ee
where $\rho_\text{rad}$ is the energy density of the target photon field.

\begin{figure}[t]
	\centering
	% adapted from Figure 5.1 from Hooper (2024)
	\includegraphics[width=0.7\columnwidth]{figures/ics-spectrum.pdf}
	\caption{The differential cross section for \ICS of a high-energy electron with a 1 eV target photon, for several values of $E_e$.
...
At higher electron energies, Klein-Nishina suppression progressively reduces the efficiency of scattering off higher-frequency photons, shifting the dominant \ICS target toward the \CMB.

\textbf{Bremsstrahlung.}
In gas-rich regions of the \ISM, electrons undergo
...
This process is the dominant cooling mechanism for cosmic-ray electrons in the MeV to GeV range in gas-dense environments such as the Galactic Plane, though it is broadly subdominant to hadronic emission in the same regions.
...
At the highest electron energies, Klein-Nishina suppression of \ICS breaks this symmetry: the \ICS loss rate falls below its Thomson-regime prediction, leaving synchrotron radiation as the primary cooling channel for the most energetic cosmic-ray electrons.
...
The following section describes how these individual components are combined into a global model of the astrophysical \GDE.

\subsection{The Galactic Diffuse Emission Model}
\label{sec:gde_model}

The individual production mechanisms described in the preceding sections combine to produce the \GDE---the brightest feature in the gamma-ray sky, accounting for roughly 80\% of all photons detected by the \textit{Fermi}-LAT \cite{Fermi-LAT:2014ryh,DGRB-review}.
...
To model this complex, spatially structured foreground, astrophysicists rely on numerical cosmic-ray propagation codes, of which GALPROP \cite{Strong:2007nh} and DRAGON \cite{Evoli:2016xgn} are two of the most established.
GALPROP, for example, starts from a set of assumptions about cosmic-ray source distributions (primarily \glspl{SNR_}), injection spectra, and diffusion parameters, and solves the steady-state diffusion-loss equation for all relevant cosmic-ray species as they propagate through the Galactic magnetic field and \ISM.
...
There, \ICS overtakes bremsstrahlung as the dominant energy-loss mechanism for electrons above a few tens of MeV, producing an extended, diffuse gamma-ray glow \cite{Pinetti:2021jjs}.
...
The Fermi collaboration applies precisely this procedure to produce the latest official Galactic interstellar emission model \texttt{gll\_iem\_v07}, which serves as the standard foreground template in Fermi-LAT analyses.
One subtlety is worth flagging: in \texttt{gll\_iem\_v07} the Fermi Bubbles and Loop~I are not modeled from dedicated physical templates but absorbed into an \emph{ad hoc} component---the \emph{patch}---built by smoothing the large-scale residuals left over after the physical templates are fit, and adding them back into the model \cite{Fermi-LAT:2019yla,Fermi-LAT:2022byn}.
Since the patch soaks up any extended residual, including the \gls{GCE_}, the official Pass~8 models are by construction ill-suited to extended-emission studies---a caveat we return to in Chapter~\ref{ch:4}.
...
\blue{A further contribution comes from \emph{dark gas}---optically thick atomic hydrogen and CO-dark molecular hydrogen, which the standard 21~cm and CO surveys miss~\cite{Grenier:2005kbp}.
This component must be traced indirectly, through dust emission or through the gamma rays themselves, and its small-scale structure therefore remains poorly constrained~\cite{Fermi-LAT:2016zaq}.}
...
This challenge recurs throughout this thesis---in the extraction of the \GCE (Chapter~\ref{ch:4}), the search for dark matter subhalos (Chapter~\ref{ch:5}), and the reconstruction of the extragalactic source-count distribution (Chapter~\ref{ch:6}).
The probabilistic and simulation-based methods developed in later chapters are designed to operate robustly in the presence of these foreground uncertainties, given they cannot be eliminated entirely.
