...
Astrophysical gamma rays from the Milky Way constitute the primary background in the search for dark matter annihilation signals.
Before cataloging individual point sources or extracting the faint extragalactic glow, one must understand the physical processes that generate this intense foreground.

This section reviews the standard gamma-ray production mechanisms.
These processes fall into two broad categories: hadronic interactions involving cosmic-ray nucleons colliding with interstellar gas, and leptonic interactions involving energetic electrons and positrons scattering off radiation fields and gas.
...
These inelastic proton-proton collisions produce secondary particles, most notably pions: \[ p + p \to \pi^0 + \pi^{\pm} + X.
\]
...
Modeling the pion production spectrum accurately requires combining different theoretical frameworks across energy regimes.
At proton kinetic energies below approximately 3 GeV, pion production is dominated by the excitation of the $\Delta_{3/2}(1232)$ baryonic resonance.
In this isobaric model, the resonance is produced collinearly with the incident proton in the center-of-momentum frame and subsequently decays isotropically in its rest frame, yielding a pion spectrum that is computed by integrating over the resonance mass distribution (described by a Breit-Wigner profile) \cite{Dermer:2009zz}.
Above 7 GeV, the number of accessible decay channels becomes too large for exclusive descriptions, and an inclusive scaling model based on Feynman scaling is adopted instead \cite{Dermer:2009zz}.
A linear interpolation smoothly joins the two formalisms in the intermediate 3--7 GeV range.
More recently, Kafexhiu et al.
\ provided a comprehensive, energy-dependent parametrization of the $pp \to \pi^0$ inclusive cross section that is widely adopted in current gamma-ray analyses \cite{Kafexhiu:2014cua}.

\begin{figure}[t]
	\centering
	% TODO: insert Figure 5.2 from Hooper (2024)
	\includegraphics[width=\columnwidth]{example-image-a}
	\caption{The inclusive cross section for inelastic pion-producing proton-proton collisions as a function of the incident proton energy.
		From \cite{Hooper:2024}, Figure~5.2.
	}
	\label{fig:pp_cross_section}
\end{figure}

While charged pions ($\pi^\pm$) decay into muons and neutrinos, neutral pions decay electromagnetically with a mean lifetime of only $\tau_{\pi^0} \approx 8.4 \times 10^{-17}$ s: \[ \pi^0 \to \gamma\gamma.
\]
...
This universal spectral feature, known as the \emph{pion bump}, is the definitive spectral signature of hadronic cosmic-ray interactions with gas and has been conclusively observed in both supernova remnants and the Galactic Plane emission \cite{Fermi-LAT:2013iui}.
...
As a result, it constitutes the primary astrophysical background for any dark matter search targeting the inner Galaxy.
Mismodeling the pion-decay component introduces systematic errors that directly impact the extraction of the Galactic Center Excess and the identification of dark matter subhalos.
...
In this regime, the ICS energy loss rate takes a form mathematically identical to that of synchrotron radiation: \[ \left.
	\frac{dE_e}{dt}\right|_\text{ICS}^\text{Thomson} = -\frac{4}{3}\,\sigma_T\,\rho_\text{rad}\,\gamma_e^2\,c,
\]
...
\begin{figure}[t]
	...
	\caption{The differential cross section for inverse Compton scattering of a high-energy electron with a 1 eV target photon, for several values of $E_e$.
		...
		From \cite{Hooper:2024}, Figure~5.1.
	}
...
In practice, cosmic-ray electrons in the local interstellar medium encounter radiation from starlight ($\rho_\text{star} \sim 0.6$ eV/cm$^3$, $T_\text{star} \sim 5000$ K), infrared emission ($\rho_\text{IR} \sim 0.6$ eV/cm$^3$, $T_\text{IR} \sim 20$ K), and the CMB ($\rho_\text{CMB} \approx 0.26$ eV/cm$^3$, $T_\text{CMB} \approx 2.7$ K).
At low electron energies, ICS losses are dominated by the higher-density optical and infrared fields.
At higher energies, however, Klein-Nishina suppression progressively reduces the efficiency of scattering off higher-frequency photons, shifting the dominant ICS target toward the CMB \cite{Hooper:2024}.

\textbf{Bremsstrahlung.}
In gas-rich regions of the interstellar medium, electrons undergo bremsstrahlung as they decelerate in the Coulomb fields of neutral and ionized atomic gas.
The bremsstrahlung cross section scales with the square of the target nuclear charge ($\propto Z^2$), making hydrogen and helium the dominant targets in the ISM.
...
The following section describes how these individual components are combined into a global model of the Galactic Diffuse Emission.

\subsection{The Galactic Diffuse Emission Model}
\label{sec:gde_model}

The individual production mechanisms described in the preceding sections combine to produce the Galactic Diffuse Emission (GDE)---the brightest feature in the gamma-ray sky, accounting for roughly 80\% of all photons detected by the \textit{Fermi}-LAT \cite{Ackermann:2014usa, DGRB-review}.
...
To model this complex, spatially structured foreground, astrophysicists rely on numerical cosmic-ray propagation codes, of which GALPROP is the most established \cite{Strong:2007nh}.
...
\begin{figure}[t]
	...
	\includegraphics[width=\columnwidth]{example-image-a}
	\caption{Energy loss coefficients for electrons and positrons at two different Galactic locations: on the Galactic Plane near the Sun (left) and 4 kpc above the Galactic Center (right).
		In the gas-rich Plane, ionization and bremsstrahlung dominate at low energies; at high latitudes, inverse Compton scattering becomes the primary loss channel above $\sim 40$ MeV.
		From \cite{Pinetti:2021jjs}, Figure~6.8.
	}
	\label{fig:energy_loss_coefficients}
\end{figure}

When analyzing \textit{Fermi}-LAT data, the standard procedure is to model the observed sky through template decomposition \cite{Ackermann:2014usa}.
...
In Chapter~\ref{ch:dnds}, extracting the faint source-count distribution requires precise control over the diffuse background model.
In Chapter~\ref{ch:dm_halos}, mismodeling the Interstellar Emission Model directly induces a \emph{dataset shift}---a systematic discrepancy between the training simulations and the real observational data---that complicates the identification of dark matter subhalos.
The advanced statistical techniques developed in later chapters, particularly simulation-based inference and robust machine learning classifiers, are largely motivated by the need to operate reliably within these foreground-dominated, systematically uncertain regimes.
