...
/referee X.Y →  critical review of the section  ← THIS SKILL
iterate on fixes
/referee X   →  chapter-level review after all sections done
```

The self-review in `section_drafting` Step 7 is a quick internal gate. This `/referee` skill is a deeper, more thorough second pass from a referee perspective.
